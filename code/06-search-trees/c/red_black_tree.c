...
int main(void) {
    Rbt t;
    rbt_init(&t);
    int seed[] = {41, 38, 31, 12, 19, 8};
    for (size_t i = 0; i < sizeof seed / sizeof seed[0]; i++) rbt_insert(&t, seed[i]);

    int out[16], n = 0;
    rbt_inorder(&t, t.root, out, &n);
    print_ints("inorder", out, n);
    printf("root: %d %s\n", t.root->key, t.root->color == BLACK ? "B" : "R");

    rbt_delete(&t, 31);
    rbt_delete(&t, 41);
    rbt_delete(&t, 8);
    n = 0; rbt_inorder(&t, t.root, out, &n);
    print_ints("delete inorder", out, n);
    return 0;
}
