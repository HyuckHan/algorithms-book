...
![A: root, degree 2.](../figures/06-search-trees/02-tree-terminology-trace-step1.svg){fig-alt="A가 주황으로 강조되고 root, degree 2라는 콜아웃이 붙은 1단계 상태" width="45%"}
...
![C의 subtree={C,F,G}; A는 G의 ancestor.](../figures/06-search-trees/02-tree-terminology-trace-step4.svg){fig-alt="C,F,G가 주황으로 강조되고 C의 subtree, A는 G의 ancestor라는 콜아웃이 붙은 마지막 상태" width="45%"}
:::
...
![A 방문. visited: A.](../figures/06-search-trees/08-preorder-trace-step1.svg){fig-alt="A만 강조되고 visited: A인 1단계 상태" width="50%"}
...
![F, G 방문. visited: A, B, D, E, C, F, G.](../figures/06-search-trees/08-preorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: A,B,D,E,C,F,G인 마지막 상태" width="50%"}
:::
...
![D 방문. visited: D.](../figures/06-search-trees/09-inorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 1단계 상태" width="50%"}
...
![F, C, G 방문. visited: D, B, E, A, F, C, G.](../figures/06-search-trees/09-inorder-trace-step4.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,B,E,A,F,C,G인 마지막 상태" width="50%"}
:::
...
![D 방문. visited: D.](../figures/06-search-trees/10-postorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 1단계 상태" width="50%"}
...
![후위 순회 완료.](../figures/06-search-trees/10-postorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,E,B,F,G,C,A인 마지막 상태" width="50%"}
:::
...
![13<15: right subtree를 버리고 left로.](../figures/06-search-trees/14-search-trace-step1.svg){fig-alt="15가 강조되고 13<15: right subtree를 버리고 left로라는 콜아웃이 붙은 1단계, 나머지 노드는 inactive" width="80%"}
...
![13=13: found.](../figures/06-search-trees/14-search-trace-step4.svg){fig-alt="13이 최종 강조되고 13=13: found라는 콜아웃이 붙은 마지막 상태, active edge 경로가 표시됨" width="80%"}
:::
...
SUCCESSOR 두 case를 같은 트리 $\{3,4,6,15,17,18,20\}$ 위에서 확인한다: 15는 right subtree(18 아래)의 minimum인 17이 successor이고, 4는 위로 올라가 처음 left-turn 경계인 6이 successor다.

::: {.panel-tabset}
## Case 1

![$succ(15)=min(18\ subtree)=17$: right subtree가 있으므로 그 안의 minimum.](../figures/06-search-trees/15-successor-cases-step1.svg){fig-alt="15와 17이 강조되고 6,3,4는 inactive로 표시된 case 1 트리 상태" width="30%"}

## Case 2

![$succ(4)=6$: 4에서 부모 3, 조부모 6으로 올라가며 처음 left child였던 지점(3)의 부모(6)가 successor.](../figures/06-search-trees/15-successor-cases-step2.svg){fig-alt="4에서 3을 거쳐 6까지 올라가는 active edge가 강조되고 15,18,17,20은 inactive로 표시된 case 2 트리 상태" width="30%"}
:::

$$succ(15)=17,\quad succ(6)=15,\quad succ(4)=6,\quad succ(20)=\NIL$$
...
![x=15, y=NIL, 12<15.](../figures/06-search-trees/17-insert-trace-step1.svg){fig-alt="15가 강조되고 x=15,y=NIL, 12&lt;15라는 콜아웃이 붙은 1단계, 나머지는 inactive" width="80%"}
...
![x=NIL, y=9. 9의 right child로 attach.](../figures/06-search-trees/17-insert-trace-step6.svg){fig-alt="9까지의 경로가 active edge로 표시되고 새 노드 12가 9의 right child로 추가된 마지막 상태" width="80%"}
:::
...
![after insert 1. edge-height=0.](../figures/06-search-trees/22-degenerate-bst-trace-step1.svg){fig-alt="1만 있는 트리, after insert 1, edge-height=0이라는 콜아웃이 붙은 1단계" width="50%"}
...
![after inserts 6, 7. edge-height=6, operations worst-case Θ(n).](../figures/06-search-trees/22-degenerate-bst-trace-step5.svg){fig-alt="1부터 7까지 오른쪽으로 계속 이어진 사슬 모양 트리, edge-height=6, worst-case Θ(n)이라는 콜아웃이 붙은 마지막 상태" width="50%"}
:::
...
![B-Tree Search(key 17): root(10|20)에서 10&lt;17&lt;20이므로 middle page 선택 → page 안에서 17 발견, 2 page accesses.](../figures/06-search-trees/48-btree-search-trace-step1.svg){fig-alt="10|20 노드가 강조되고 10&lt;17&lt;20 middle page 선택이라는 콜아웃이 붙은 1단계, 자식 5|6|7, 12|17, 30 표시" width="80%"}

![](../figures/06-search-trees/48-btree-search-trace-step2.svg){fig-alt="12|17 노드가 강조되고 page 안에서 17 발견, 2 page accesses라는 콜아웃이 붙은 마지막 상태" width="80%"}
