...
![A 방문. visited: A.](../figures/06-search-trees/08-preorder-trace-step1.svg){fig-alt="A만 강조되고 visited: A인 1단계 상태" width="40%"}

## 2단계

![B 방문. visited: A, B.](../figures/06-search-trees/08-preorder-trace-step2.svg){fig-alt="A는 처리 완료로, B가 강조되어 visited: A, B인 상태" width="40%"}

## 3단계

![D, E 방문(왼쪽 먼저 끝까지). visited: A, B, D, E.](../figures/06-search-trees/08-preorder-trace-step3.svg){fig-alt="A,B는 처리 완료로, D,E가 강조되어 visited: A, B, D, E인 상태" width="40%"}

## 4단계

![C 방문. visited: A, B, D, E, C.](../figures/06-search-trees/08-preorder-trace-step4.svg){fig-alt="A,B,D,E는 처리 완료로, C가 강조되어 visited: A, B, D, E, C인 상태" width="40%"}

## 5단계

![F, G 방문. visited: A, B, D, E, C, F, G.](../figures/06-search-trees/08-preorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: A,B,D,E,C,F,G인 마지막 상태" width="40%"}
:::
...
![D 방문. visited: D.](../figures/06-search-trees/09-inorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 1단계 상태" width="40%"}

## 2단계

![B, E 방문. visited: D, B, E.](../figures/06-search-trees/09-inorder-trace-step2.svg){fig-alt="D는 처리 완료로, B,E가 강조되어 visited: D, B, E인 상태" width="40%"}

## 3단계

![A 방문. visited: D, B, E, A.](../figures/06-search-trees/09-inorder-trace-step3.svg){fig-alt="D,B,E는 처리 완료로, A가 강조되어 visited: D, B, E, A인 상태" width="40%"}

## 4단계

![F, C, G 방문. visited: D, B, E, A, F, C, G.](../figures/06-search-trees/09-inorder-trace-step4.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,B,E,A,F,C,G인 마지막 상태" width="40%"}
:::
...
![D 방문. visited: D.](../figures/06-search-trees/10-postorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 1단계 상태" width="40%"}

## 2단계

![E, B 방문. visited: D, E, B.](../figures/06-search-trees/10-postorder-trace-step2.svg){fig-alt="D는 처리 완료로, E,B가 강조되어 visited: D, E, B인 상태" width="40%"}

## 3단계

![F, G, C 방문. visited: D, E, B, F, G, C.](../figures/06-search-trees/10-postorder-trace-step3.svg){fig-alt="D,E,B는 처리 완료로, F,G,C가 강조되어 visited: D, E, B, F, G, C인 상태" width="40%"}

## 4단계

![A 방문. visited: D, E, B, F, G, C, A.](../figures/06-search-trees/10-postorder-trace-step4.svg){fig-alt="A만 남아 강조되고 나머지는 처리 완료로 표시된 상태" width="40%"}

## 5단계

![후위 순회 완료.](../figures/06-search-trees/10-postorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,E,B,F,G,C,A인 마지막 상태" width="40%"}
:::
...
![13<15: right subtree를 버리고 left로.](../figures/06-search-trees/14-search-trace-step1.svg){fig-alt="15가 강조되고 13<15: right subtree를 버리고 left로라는 콜아웃이 붙은 1단계, 나머지 노드는 inactive" width="55%"}

## 2단계

![13>6: left subtree를 버리고 right로.](../figures/06-search-trees/14-search-trace-step2.svg){fig-alt="6이 강조되고 13>6: left subtree를 버리고 right로라는 콜아웃이 붙은 상태, 15에서 6까지 active edge 표시" width="55%"}

## 3단계

![13>7: left/NIL을 버리고 right로.](../figures/06-search-trees/14-search-trace-step3.svg){fig-alt="7이 강조되고 13>7: left/NIL을 버리고 right로라는 콜아웃이 붙은 상태, 15에서 7까지 active edge 표시" width="55%"}

## 4단계

![13=13: found.](../figures/06-search-trees/14-search-trace-step4.svg){fig-alt="13이 최종 강조되고 13=13: found라는 콜아웃이 붙은 마지막 상태, active edge 경로가 표시됨" width="55%"}
:::
...
![x=15, y=NIL, 12<15.](../figures/06-search-trees/17-insert-trace-step1.svg){fig-alt="15가 강조되고 x=15,y=NIL, 12&lt;15라는 콜아웃이 붙은 1단계, 나머지는 inactive" width="55%"}

## 2단계

![x=6, y=15, 12>6.](../figures/06-search-trees/17-insert-trace-step2.svg){fig-alt="6이 강조되고 x=6,y=15, 12&gt;6이라는 콜아웃이 붙은 상태, 15에서 6까지 active edge 표시" width="55%"}

## 3단계

![x=7, y=6, 12>7.](../figures/06-search-trees/17-insert-trace-step3.svg){fig-alt="7이 강조되고 x=7,y=6, 12&gt;7이라는 콜아웃이 붙은 상태, 6에서 7까지 active edge 표시" width="55%"}

## 4단계

![x=13, y=7, 12<13.](../figures/06-search-trees/17-insert-trace-step4.svg){fig-alt="13이 강조되고 x=13,y=7, 12&lt;13이라는 콜아웃이 붙은 상태, 7에서 13까지 active edge 표시" width="55%"}

## 5단계

![x=9, y=13, 12>9.](../figures/06-search-trees/17-insert-trace-step5.svg){fig-alt="9가 강조되고 x=9,y=13, 12&gt;9라는 콜아웃이 붙은 상태, 13에서 9까지 active edge 표시" width="55%"}

## 6단계

![x=NIL, y=9. 9의 right child로 attach.](../figures/06-search-trees/17-insert-trace-step6.svg){fig-alt="9까지의 경로가 active edge로 표시되고 새 노드 12가 9의 right child로 추가된 마지막 상태" width="55%"}
:::
