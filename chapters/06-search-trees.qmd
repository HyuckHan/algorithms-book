...
![Rooted tree 예제: A가 root, B와 C가 A의 child, D·E가 B의 child, F·G가 C의 child다.](../figures/06-search-trees/01-rooted-tree-definition.svg){fig-alt="A를 root로 B,C가 자식이고 B의 자식이 D,E, C의 자식이 F,G인 트리 다이어그램" width="25%"}
...
Tree 용어 추적: 같은 트리에서 root/degree → parent/child/sibling → leaf/internal → subtree/ancestor 순으로 하이라이트를 옮긴다.

::: {.panel-tabset}
## 1단계

![A: root, degree 2.](../figures/06-search-trees/02-tree-terminology-trace-step1.svg){fig-alt="A가 주황으로 강조되고 root, degree 2라는 콜아웃이 붙은 상태" width="45%"}

## 2단계

![A는 parent; B와 C는 child이자 sibling.](../figures/06-search-trees/02-tree-terminology-trace-step2.svg){fig-alt="B와 C가 강조되고 A는 parent, B·C는 child이자 sibling이라는 콜아웃이 붙은 상태" width="45%"}

## 3단계

![D와 E는 leaf; B는 internal node.](../figures/06-search-trees/02-tree-terminology-trace-step3.svg){fig-alt="D,E가 강조되고 D·E는 leaf, B는 internal node라는 콜아웃이 붙은 상태" width="45%"}

## 4단계

![C의 subtree={C,F,G}; A는 G의 ancestor.](../figures/06-search-trees/02-tree-terminology-trace-step4.svg){fig-alt="C,F,G가 주황으로 강조되고 C의 subtree, A는 G의 ancestor라는 콜아웃이 붙은 상태" width="45%"}
:::
...
![A의 left child가 B인 경우.](../figures/06-search-trees/03-left-child-example.svg){fig-alt="A에서 B로 L 표시가 붙은 edge가 내려가는 다이어그램" width="10%"}

![A의 right child가 B인 경우.](../figures/06-search-trees/04-right-child-example.svg){fig-alt="A에서 B로 R 표시가 붙은 edge가 내려가는 다이어그램" width="10%"}
...
![Linked node: parent/key/left/right 네 필드를 가진 레코드.](../figures/06-search-trees/06-linked-representation.svg){fig-alt="parent, key, left, right 네 칸으로 나뉜 레코드 다이어그램" width="25%"}
...
Preorder 5단계: A 방문 → B 방문 → D,E 방문(왼쪽 먼저 끝까지) → C 방문 → F,G 방문.

::: {.panel-tabset}
## 1단계

![A 방문. visited: A.](../figures/06-search-trees/08-preorder-trace-step1.svg){fig-alt="A만 강조되고 visited: A인 상태" width="50%"}

## 2단계

![B 방문. visited: A, B.](../figures/06-search-trees/08-preorder-trace-step2.svg){fig-alt="A는 처리 완료로, B가 강조되어 visited: A, B인 상태" width="50%"}

## 3단계

![D, E 방문(왼쪽 먼저 끝까지). visited: A, B, D, E.](../figures/06-search-trees/08-preorder-trace-step3.svg){fig-alt="A,B는 처리 완료로, D,E가 강조되어 visited: A, B, D, E인 상태" width="50%"}

## 4단계

![C 방문. visited: A, B, D, E, C.](../figures/06-search-trees/08-preorder-trace-step4.svg){fig-alt="A,B,D,E는 처리 완료로, C가 강조되어 visited: A, B, D, E, C인 상태" width="50%"}

## 5단계

![F, G 방문. visited: A, B, D, E, C, F, G.](../figures/06-search-trees/08-preorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: A,B,D,E,C,F,G인 상태" width="50%"}
:::
...
Inorder 4단계: D 방문 → B,E 방문 → A 방문 → F,C,G 방문.

::: {.panel-tabset}
## 1단계

![D 방문. visited: D.](../figures/06-search-trees/09-inorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 상태" width="50%"}

## 2단계

![B, E 방문. visited: D, B, E.](../figures/06-search-trees/09-inorder-trace-step2.svg){fig-alt="D는 처리 완료로, B,E가 강조되어 visited: D, B, E인 상태" width="50%"}

## 3단계

![A 방문. visited: D, B, E, A.](../figures/06-search-trees/09-inorder-trace-step3.svg){fig-alt="D,B,E는 처리 완료로, A가 강조되어 visited: D, B, E, A인 상태" width="50%"}

## 4단계

![F, C, G 방문. visited: D, B, E, A, F, C, G.](../figures/06-search-trees/09-inorder-trace-step4.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,B,E,A,F,C,G인 상태" width="50%"}
:::
...
Postorder 5단계: D 방문 → E,B 방문 → F,G,C 방문 → A 방문.

::: {.panel-tabset}
## 1단계

![D 방문. visited: D.](../figures/06-search-trees/10-postorder-trace-step1.svg){fig-alt="D만 강조되고 visited: D인 상태" width="50%"}

## 2단계

![E, B 방문. visited: D, E, B.](../figures/06-search-trees/10-postorder-trace-step2.svg){fig-alt="D는 처리 완료로, E,B가 강조되어 visited: D, E, B인 상태" width="50%"}

## 3단계

![F, G, C 방문. visited: D, E, B, F, G, C.](../figures/06-search-trees/10-postorder-trace-step3.svg){fig-alt="D,E,B는 처리 완료로, F,G,C가 강조되어 visited: D, E, B, F, G, C인 상태" width="50%"}

## 4단계

![A 방문. visited: D, E, B, F, G, C, A.](../figures/06-search-trees/10-postorder-trace-step4.svg){fig-alt="A만 남아 강조되고 나머지는 처리 완료로 표시된 상태" width="50%"}

## 5단계

![후위 순회 완료.](../figures/06-search-trees/10-postorder-trace-step5.svg){fig-alt="모든 노드가 처리 완료로 표시되고 visited: D,E,B,F,G,C,A인 상태" width="50%"}
:::
...
![기준 tree(level-order queue animation과 함께 참고).](../figures/06-search-trees/11-level-order-reference-tree.svg){fig-alt="A,B,C,D,E,F,G 고정 tree" width="50%"}
...
BinaryTree의 API contract: generic data는 non-null, empty tree height $=-1$, leaf height $=0$이다. constructor는 subtree를 **deep copy**해 ownership을 분리한다 — subtree root를 공유하면 한 tree의 변경이 다른 tree에 보일 수 있기 때문이다. traversal은 immutable `List<E>`를 반환한다.

$$\operatorname{size}(x)=1+\operatorname{size}(x.left)+\operatorname{size}(x.right)$$

$$\operatorname{height}(x)=1+\max(h(x.left),h(x.right)),\ h(\NIL)=-1$$
...
SEARCH 13: root에서 시작해 13<15로 left, 13>6로 right, 13>7로 right, 13=13으로 종료(4단계).

::: {.panel-tabset}
## 1단계

![13<15: right subtree를 버리고 left로.](../figures/06-search-trees/14-search-trace-step1.svg){fig-alt="15가 강조되고 13<15: right subtree를 버리고 left로라는 콜아웃이 붙은 상태, 나머지 노드는 inactive" width="80%"}

## 2단계

![13>6: left subtree를 버리고 right로.](../figures/06-search-trees/14-search-trace-step2.svg){fig-alt="6이 강조되고 13>6: left subtree를 버리고 right로라는 콜아웃이 붙은 상태, 15에서 6까지 active edge 표시" width="80%"}

## 3단계

![13>7: left/NIL을 버리고 right로.](../figures/06-search-trees/14-search-trace-step3.svg){fig-alt="7이 강조되고 13>7: left/NIL을 버리고 right로라는 콜아웃이 붙은 상태, 15에서 7까지 active edge 표시" width="80%"}

## 4단계

![13=13: found.](../figures/06-search-trees/14-search-trace-step4.svg){fig-alt="13이 최종 강조되고 13=13: found라는 콜아웃이 붙은 상태, active edge 경로가 표시됨" width="80%"}
:::
...
SUCCESSOR 두 case를 이 장의 고정 BST 예제 위에서 확인한다: 15는 right subtree(18 아래)의 minimum인 17이 successor이고, 4는 위로 올라가 처음 left-turn 경계인 6이 successor다.

::: {.panel-tabset}
## Case 1

![$succ(15)=min(18\ subtree)=17$: right subtree가 있으므로 그 안의 minimum.](../figures/06-search-trees/15-successor-cases-step1.svg){fig-alt="12-node 고정 BST에서 15와 17이 강조되고 18은 방문 표시, 나머지 노드는 inactive로 표시된 case 1 트리 상태" width="55%"}

## Case 2

![$succ(4)=6$: 4에서 부모 3, 조부모 6으로 올라가며 처음 left child였던 지점(3)의 부모(6)가 successor.](../figures/06-search-trees/15-successor-cases-step2.svg){fig-alt="12-node 고정 BST에서 4에서 3을 거쳐 6까지 올라가는 active edge가 강조되고 나머지 노드는 inactive로 표시된 case 2 트리 상태" width="55%"}
:::
...
INSERT 12: x=15,y=NIL(12&lt;15) → x=6,y=15(12&gt;6) → x=7,y=6(12&gt;7) → x=13,y=7(12&lt;13) → x=9,y=13(12&gt;9) → x=NIL,y=9, 9의 right child로 attach(6단계, 절대 경로 15→6→7→13→9→NIL).

::: {.panel-tabset}
## 1단계

![x=15, y=NIL, 12<15.](../figures/06-search-trees/17-insert-trace-step1.svg){fig-alt="15가 강조되고 x=15,y=NIL, 12&lt;15라는 콜아웃이 붙은 상태, 나머지는 inactive" width="80%"}

## 2단계

![x=6, y=15, 12>6.](../figures/06-search-trees/17-insert-trace-step2.svg){fig-alt="6이 강조되고 x=6,y=15, 12&gt;6이라는 콜아웃이 붙은 상태, 15에서 6까지 active edge 표시" width="80%"}

## 3단계

![x=7, y=6, 12>7.](../figures/06-search-trees/17-insert-trace-step3.svg){fig-alt="7이 강조되고 x=7,y=6, 12&gt;7이라는 콜아웃이 붙은 상태, 6에서 7까지 active edge 표시" width="80%"}

## 4단계

![x=13, y=7, 12<13.](../figures/06-search-trees/17-insert-trace-step4.svg){fig-alt="13이 강조되고 x=13,y=7, 12&lt;13이라는 콜아웃이 붙은 상태, 7에서 13까지 active edge 표시" width="80%"}

## 5단계

![x=9, y=13, 12>9.](../figures/06-search-trees/17-insert-trace-step5.svg){fig-alt="9가 강조되고 x=9,y=13, 12&gt;9라는 콜아웃이 붙은 상태, 13에서 9까지 active edge 표시" width="80%"}

## 6단계

![x=NIL, y=9. 9의 right child로 attach.](../figures/06-search-trees/17-insert-trace-step6.svg){fig-alt="9까지의 경로가 active edge로 표시되고 새 노드 12가 9의 right child로 추가된 상태" width="80%"}
:::
...
![7이 제거되고 parent의 자식 자리가 NIL이 된다 — leaf 삭제는 재배치가 필요 없다.](../figures/06-search-trees/18-delete-case1-leaf-step2.svg){fig-alt="7 자리가 NIL로 바뀐 삭제 후 상태" width="35%"}
...
![6이 있던 자리에 subtree 3이 그대로 올라온다 — 자식이 하나면 그 subtree를 통째로 끌어올리면 된다.](../figures/06-search-trees/19-delete-case2-one-child-step2.svg){fig-alt="3이 6의 자리로 직접 연결된 삭제 후 상태" width="35%"}

**DELETE Case 3**(15 삭제, successor 17 선택): $y=min(15.right)=17$이고 따라서 $y.left=\NIL$이다.

![DELETE Case 3 준비: target 15, successor 후보 $y=17$(20의 left child), 20의 right child 25, 17의 right child 18.](../figures/06-search-trees/20-delete-case3-successor-setup.svg){fig-alt="15가 target, 17이 current로 표시되고 15의 왼쪽에 6, 오른쪽에 20(자식 17,25), 17의 자식 18이 있는 다이어그램" width="70%"}
...
![DELETE Case 3 완료: 17이 새 root, inorder는 6,17,18,20,25로 여전히 정렬돼 있다.](../figures/06-search-trees/21-delete-case3-final-tree.svg){fig-alt="17이 새 루트가 되고 왼쪽에 6, 오른쪽에 20(자식 18,25)이 연결된 최종 트리" width="70%"}
...
**답**: 1. Transplant의 root branch. 
2. right subtree minimum이므로. 
3. $\Theta(h)$.
:::
...
BST 연산은 모두 $\Theta(h)$다 — $h$를 구하는 재귀식 분석 도구(재귀 트리·반복대치)는 재귀 장의 것을 그대로 쓴다. 문제는 height $h$ 자체가 입력 순서에 따라 얼마든지 커질 수 있다는 점이다 — 정렬 장의 Quick Sort가 이미 정렬되거나 치우친 입력에서 partition이 한쪽으로 쏠려 worst-case $\Theta(n^2)$이 되는 것과 같은 현상이다: 정렬된 입력을 순서대로 삽입하면 BST도 매번 한쪽으로만 자라 tree가 사실상 linked list($h=\Theta(n)$)가 된다.
:::

정렬된 키 `1,2,3,4,5,6,7`을 순서대로 삽입하면 매번 새 key가 가장 오른쪽에 붙어 height가 계속 늘어난다.

정렬 입력 1..7 삽입 5단계: 매번 오른쪽 끝에 새 node가 붙어 결국 edge-height 6인 사슬(chain)이 된다.

::: {.panel-tabset}
## 1단계

![after insert 1. edge-height=0.](../figures/06-search-trees/22-degenerate-bst-trace-step1.svg){fig-alt="1만 있는 트리, after insert 1, edge-height=0이라는 콜아웃이 붙은 상태" width="50%"}

## 2단계

![after insert 2. edge-height=1.](../figures/06-search-trees/22-degenerate-bst-trace-step2.svg){fig-alt="1-2 사슬 트리, after insert 2, edge-height=1이라는 콜아웃이 붙은 상태" width="50%"}

## 3단계

![after insert 3. search path가 계속 길어짐.](../figures/06-search-trees/22-degenerate-bst-trace-step3.svg){fig-alt="1-2-3 사슬 트리, after insert 3, search path가 계속 길어진다는 콜아웃이 붙은 상태" width="50%"}

## 4단계

![after inserts 4, 5. latest node 5.](../figures/06-search-trees/22-degenerate-bst-trace-step4.svg){fig-alt="1부터 5까지 이어진 사슬 트리, after inserts 4,5, latest node 5라는 콜아웃이 붙은 상태" width="50%"}

## 5단계

![after inserts 6, 7. edge-height=6, operations worst-case Θ(n).](../figures/06-search-trees/22-degenerate-bst-trace-step5.svg){fig-alt="1부터 7까지 오른쪽으로 계속 이어진 사슬 모양 트리, edge-height=6, worst-case Θ(n)이라는 콜아웃이 붙은 상태" width="50%"}
:::
...
이 장은 right-left 대신 **$BF=h(left)-h(right)$**(left-right)를 쓴다. 모든 case와 코드가 이 부호를 따른다.
:::
...
![Right rotation 전: y가 root, x가 y의 left child, C가 y의 right subtree, A·B가 x의 left·right subtree.](../figures/06-search-trees/23-right-rotation-before.svg){fig-alt="y를 루트로 왼쪽에 x(자식 A,B), 오른쪽에 C가 있는 트리" width="55%"}

![Right rotation 후: x가 새 root, A가 x의 left, y가 x의 right(자식 B,C). inorder는 A,x,B,y,C로 그대로다.](../figures/06-search-trees/24-right-rotation-after.svg){fig-alt="x를 루트로 왼쪽에 A, 오른쪽에 y(자식 B,C)가 있는 트리" width="50%"}
...
![LL 교정 후: 20이 root(BF=0), 10·30이 좌우 child.](../figures/06-search-trees/26-ll-case-after.svg){fig-alt="20을 루트로 10,30이 자식인 균형 잡힌 트리" width="25%"}
...
![RR 교정 후: 20이 root(BF=0), 10·30이 좌우 child.](../figures/06-search-trees/28-rr-case-after.svg){fig-alt="20을 루트로 10,30이 자식인 균형 잡힌 트리" width="25%"}
...
![LR 1단계: 30(BF=+2)의 left child 10, 10의 right child 20 — 안쪽 shape를 확인한다.](../figures/06-search-trees/29-lr-case-step1.svg){fig-alt="30이 위반, 자식 10(강조), 10의 자식 20인 트리" width="15%"}

![LR 2단계: 10을 left-rotate해 LL 형태로 바꾼다.](../figures/06-search-trees/30-lr-case-step2.svg){fig-alt="30 아래 20(강조), 20의 자식 10으로 바뀐 중간 상태" width="25%"}

![LR 3단계: 30을 right-rotate해 완료.](../figures/06-search-trees/31-lr-case-step3.svg){fig-alt="20을 루트로 10,30이 자식인 균형 잡힌 트리" width="25%"}

**RL case**(double rotation): `RIGHT-ROTATE`$(30)$ 후 `LEFT-ROTATE`$(10)$.

![RL 1단계: 10(BF=-2)의 right child 30, 30의 left child 20 — 안쪽 shape를 확인한다.](../figures/06-search-trees/32-rl-case-step1.svg){fig-alt="10이 위반, 자식 30(강조), 30의 자식 20인 트리" width="15%"}

![RL 2단계: 30을 right-rotate해 RR 형태로 바꾼다.](../figures/06-search-trees/33-rl-case-step2.svg){fig-alt="10 아래 20(강조), 20의 자식 30으로 바뀐 중간 상태" width="25%"}

![RL 3단계: 10을 left-rotate해 완료.](../figures/06-search-trees/34-rl-case-step3.svg){fig-alt="20을 루트로 10,30이 자식인 균형 잡힌 트리" width="25%"}
...
![NIL sentinel: 10(B)의 두 child 5(R)·15(R)가 각각 NIL(sentinel) 두 개씩을 가리킨다.](../figures/06-search-trees/35-nil-sentinel.svg){fig-alt="10B를 루트로 5R,15R이 자식이고 각각 NIL 사각형 두 개를 가리키는 트리" width="40%"}
...
![41,38 삽입 후 31 삽입 직전: 41(B) 아래 38(R).](../figures/06-search-trees/39-rb-insert-41-38-31-step1.svg){fig-alt="41B 아래 38R 하나만 있는 트리" width="15%"}

![31 삽입 직후: 38(R) 아래 31(R) — uncle black, outer.](../figures/06-search-trees/40-rb-insert-41-38-31-step2.svg){fig-alt="41B 아래 38R, 38의 자식 31R인 트리, uncle black outer라는 콜아웃" width="40%"}

![right rotate 41 후: 38(B)이 root, 31·41이 red 자식.](../figures/06-search-trees/41-rb-insert-41-38-31-step3.svg){fig-alt="38B를 루트로 31R,41R이 자식인 트리, right rotate 41이라는 콜아웃" width="40%"}
...
![recolor 후: 31,41이 B로, 38은 이미 root라 그대로 B.](../figures/06-search-trees/43-rb-insert-12-after.svg){fig-alt="38B 아래 31B(자식 12R), 41B로 바뀐 트리, uncle red recolor라는 콜아웃" width="50%"}
...
![최종 tree(41,38,31,12,19,8 삽입 완료): root 38(B), 모든 invariant 충족.](../figures/06-search-trees/46-rb-insert-8-final.svg){fig-alt="38B를 루트로 19R(자식 12B(자식 8R), 31B), 41B인 최종 트리" width="50%"}
...
RB Tree가 delete를 지원하지만, 이 장의 목표는 **insertion fixup**이다. sibling color와 near/far nephew에 따라 case가 더 많아지며, 상세 fixup 로직은 실제 구현 코드(아래 panel-tabset)와 Appendix 수준에서만 다룬다.
:::
...
![B-Tree Search(key 17): root(10|20)에서 10&lt;17&lt;20이므로 middle page로 내려간다.](../figures/06-search-trees/48-btree-search-trace-step1.svg){fig-alt="10|20 노드가 강조되고 10&lt;17&lt;20 middle page 선택이라는 콜아웃이 붙은 상태, 자식 5|6|7, 12|17, 30 표시" width="80%"}

![middle page(12|17) 안에서 17을 찾는다 — root와 이 page, 2 page accesses로 끝난다.](../figures/06-search-trees/48-btree-search-trace-step2.svg){fig-alt="12|17 노드가 강조되고 page 안에서 17 발견, 2 page accesses라는 콜아웃이 붙은 상태" width="80%"}
...
![10 삽입: root가 [10]인 상태.](../figures/06-search-trees/49-btree-insert-step1.svg){fig-alt="10 하나만 있는 노드" width="10%"}

![20 삽입: root가 [10,20]인 상태.](../figures/06-search-trees/50-btree-insert-step2.svg){fig-alt="10,20 두 키가 있는 노드" width="15%"}

![5 삽입 후 root가 [5,10,20]로 가득 참(root full).](../figures/06-search-trees/51-btree-insert-step3.svg){fig-alt="5,10,20 세 키로 가득 찬 노드, root full이라는 콜아웃" width="40%"}

![10 promotion 후 6 삽입: 10이 새 root, [5,6]과 [20]이 두 child.](../figures/06-search-trees/52-btree-insert-step4.svg){fig-alt="10을 루트로 5,6과 20 두 child로 분리된 트리, 10 promotion 후 6 삽입이라는 콜아웃" width="70%"}
...
![split-before-descend로 20 up, 17 삽입 후: root [10,20], child [5,6,7]/[12,17]/[30].](../figures/06-search-trees/54-btree-insert2-step2.svg){fig-alt="10,20을 루트로 5,6,7 / 12,17 / 30 세 child로 나뉜 트리, split before descent 20 up then insert 17이라는 콜아웃" width="80%"}
...
![Borrow from right sibling 전: [5]가 $t-1$ key.](../figures/06-search-trees/56-btree-borrow-before.svg){fig-alt="10,20을 루트로 5, 12|15, 30 세 child가 있고 5가 t-1 keys라는 콜아웃이 붙은 상태" width="80%"}

![Borrow 후: parent key 10이 내려가고 sibling의 최솟값 12가 올라와 [5,10]과 [15]가 된다.](../figures/06-search-trees/57-btree-borrow-after.svg){fig-alt="12,20을 루트로 5,10 / 15 / 30 세 child로 바뀐 상태, after borrowing이라는 콜아웃" width="80%"}

parent key 10이 내려가고 right sibling의 가장 작은 key 12가 올라온다(separator movement).

![Merge 전: 둘 다 $t-1$ key.](../figures/06-search-trees/58-btree-merge-before.svg){fig-alt="10을 루트로 5, 15 두 child가 있고 둘 다 t-1 keys라는 콜아웃이 붙은 상태" width="70%"}

![Merge 후: root가 비어 유일한 child가 새 root가 된다.](../figures/06-search-trees/59-btree-merge-after.svg){fig-alt="5,10,15 세 키를 가진 단일 노드로 병합된 상태, merge 후 old root는 empty, 유일한 child가 new root라는 콜아웃" width="50%"}
...
**답**: 1. 2–5. 
2. worst-case $\Theta(\log_t n)$ page I/O. 
3. parent key는 child로 내려가고 sibling key는 올라온다. 
4. 유일한 child가 새 root가 된다.
:::
...
**실행 결과** (실제 컴파일·실행 캡처 — `gcc -Wall`, `javac`/`java`, `python3`): 8개 삽입 직후의 `size: 8`이 `inorder`의 8개 값과 같은 시점이고, root `[10,20]`·child `[5,6,7]/[12,17]/[30]`이 위 "최종 B-Tree invariant 확인" 그림과 일치한다. 마지막 `delete size: 0`은 이어서 8개를 모두 삭제한 **다른 시점**의 상태다.
...
2. RB Tree. 
3. B-Tree. 
4. balance guarantee가 없으므로 피하거나 self-balancing tree를 쓴다.
:::

## 개념 지도

![Search Trees 개념 전체: BST(Θ(h)), Balanced(AVL/RB, rotation invariant), B-Tree(page index, split/borrow/merge)로 정리한다.](../figures/06-search-trees/60-concept-map.svg){fig-alt="Search Trees를 중심으로 BST, Balanced(AVL/RB), B-Tree 세 갈래로 나뉘고 각각 order traversal, rotation invariant, split borrow/merge로 이어지는 개념 지도" width="55%"}
