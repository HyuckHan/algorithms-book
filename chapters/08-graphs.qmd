...
description: "그래프 표현부터 BFS/DFS, 위상 정렬, 최소 신장 트리(Prim/Kruskal), 최단 경로(Dijkstra/Bellman-Ford)까지 — 3장의 heap과 5장의 DP를 다시 불러온다."
lecture: 8
order: 5
slug: "08-graphs"
status: "verified"
...
![$A[0][1]=1$은 $0\to1$ edge를 뜻한다 — 행은 source, 열은 destination.](../figures/08-graphs/02-matrix-direction.svg){fig-alt="0에서 1로 향하는 방향 edge와 그 옆에 2x2 인접행렬이 나란히 표시된 다이어그램" width="40%"}
...
![Row 0: $0,1,1,0$ — nonzero column을 찾는다.](../figures/08-graphs/03-matrix-to-list-step1.svg){fig-alt="matrix에서 row 0의 값을 나열하고 nonzero column을 찾는 설명 콜아웃" width="70%"}

## 2단계

![$\operatorname{Adj}[0]=[1,2]$ — row 1, 2, 3도 같은 방식으로 반복한다.](../figures/08-graphs/03-matrix-to-list-step2.svg){fig-alt="Adj[0]이 [1,2]로 채워지고 나머지 row도 반복한다는 콜아웃" width="70%"}

## 3단계

![matrix 스캔은 $\Theta(V^2)$, list 저장은 $\Theta(V+E)$.](../figures/08-graphs/03-matrix-to-list-step3.svg){fig-alt="matrix scan 비용과 list storage 비용을 비교하는 콜아웃" width="70%"}
:::
...
![BFS를 추적할 고정 예제 그래프(8 vertex, 9 edge).](../figures/08-graphs/04-bfs-graph.svg){fig-alt="0부터 7까지 8개 vertex가 세 개의 열로 배치되고 9개 edge로 연결된 그래프" width="70%"}
...
![start 0. Queue: 0.](../figures/08-graphs/05-bfs-queue-trace-step1.svg){fig-alt="큐에 0만 있고 아직 아무것도 처리되지 않은 상태" width="22%"}
...
![dequeue 1; discover 4. Queue: 2,3,4. Output: 0,1.](../figures/08-graphs/05-bfs-queue-trace-step3.svg){fig-alt="1이 dequeue되고 4가 큐에 들어간 상태" width="22%"}

## 4단계

![dequeue 2; discover 5. Queue: 3,4,5. Output: 0,1,2.](../figures/08-graphs/05-bfs-queue-trace-step4.svg){fig-alt="2가 dequeue되고 5가 큐에 들어간 상태" width="22%"}

## 5단계

![dequeue 3; discover 6. Queue: 4,5,6. Output: 0,1,2,3.](../figures/08-graphs/05-bfs-queue-trace-step5.svg){fig-alt="3이 dequeue되고 6이 큐에 들어간 상태" width="22%"}
...
![discover 0. Stack: 0.](../figures/08-graphs/07-dfs-recursion-trace-step1.svg){fig-alt="0만 discover되어 스택에 있는 상태" width="22%"}
...
![finish 5, then 2. Discovery: 0,1,4,2,5.](../figures/08-graphs/07-dfs-recursion-trace-step3.svg){fig-alt="5와 2가 finish되어 스택에서 제거된 상태" width="22%"}
...
![Zero heap: [0]. Select: --. Output: [].](../figures/08-graphs/09-kahn-trace-step1.svg){fig-alt="zero-indegree heap에 0만 있는 초기 상태" width="22%"}

## 2단계

![Zero heap: [1,3]. Select: 0. Output: [0].](../figures/08-graphs/09-kahn-trace-step2.svg){fig-alt="0이 선택되고 1,3이 heap에 들어간 상태" width="22%"}

## 3단계

![Zero heap: [2,3,4]. Select: 1. Output: [0,1].](../figures/08-graphs/09-kahn-trace-step3.svg){fig-alt="1이 선택되고 2,4가 heap에 추가된 상태" width="22%"}

## 4단계

![Zero heap: [3,4]. Select: 2. Output: [0,1,2].](../figures/08-graphs/09-kahn-trace-step4.svg){fig-alt="2가 선택된 상태" width="22%"}

## 5단계

![Zero heap: [4]. Select: 3. Output: [0,1,2,3].](../figures/08-graphs/09-kahn-trace-step5.svg){fig-alt="3이 선택된 상태" width="22%"}

## 6단계

![Zero heap: [5]→[]. Select: 4 then 5. Output: [0,1,2,3,4,5].](../figures/08-graphs/09-kahn-trace-step6.svg){fig-alt="4와 5가 차례로 선택되어 heap이 빈 최종 상태" width="22%"}
:::
...
![Finish append: 5.](../figures/08-graphs/11-dfs-topo-finish-reverse-step1.svg){fig-alt="finish 순서에 5만 기록된 상태" width="70%"}

## 2단계

![Finish append: 5,2.](../figures/08-graphs/11-dfs-topo-finish-reverse-step2.svg){fig-alt="finish 순서에 5,2가 기록된 상태" width="70%"}

## 3단계

![Finish append: 5,2,4,1.](../figures/08-graphs/11-dfs-topo-finish-reverse-step3.svg){fig-alt="finish 순서에 5,2,4,1이 기록된 상태" width="70%"}

## 4단계

![Finish append: 5,2,4,1,3,0. Reverse: 0,3,1,4,2,5.](../figures/08-graphs/11-dfs-topo-finish-reverse-step4.svg){fig-alt="finish 순서가 완성되고 뒤집은 최종 위상 순서가 표시된 상태" width="70%"}
:::
...
(**5장(동적 계획법)의 DP evaluation order**와도 같은 개념이다 — Part M에서 다시 짚는다).
...
![$\{A,B\}$와 $\{C,D\}$ 사이 cut을 건너는 light edge $B$–$C$.](../figures/08-graphs/12-cut-crossing-edge.svg){fig-alt="A-B, C-D가 각 그룹 안에서 연결되고 B에서 C로 향하는 crossing edge가 강조되며 그 사이에 cut 선이 그려진 다이어그램" width="55%"}
...
::: {.callout-tip title="3장의 heap이 여기서 돌아온다"}
Prim의 $Q$와(뒤에서 볼) Dijkstra의 우선순위 큐는 새 자료구조가 아니라 **3장(정렬)에서 다룬
...
전체 vertex $V$개를 꺼내고 edge $E$개마다 갱신하므로 $O((V+E)\log V)$다. 같은 heap이 10장의
A\*에서는 $f=g+h$를 key로 하는 min-PQ로 다시 등장한다.
:::
...
![Extract: B. Update: E=10/B. Selected: AB8.](../figures/08-graphs/13-prim-trace-step1.svg){fig-alt="A-G 그래프에서 B가 추출되고 AB edge가 MST에 추가된 상태" width="80%"}

## 2단계

![Extract: C. Update: E=5/C, F=12/C. Selected: AC9.](../figures/08-graphs/13-prim-trace-step2.svg){fig-alt="C가 추출되고 AC edge가 추가된 상태" width="80%"}

## 3단계

![Extract: E. Update: none. Selected: CE5.](../figures/08-graphs/13-prim-trace-step3.svg){fig-alt="E가 추출되고 CE edge가 추가된 상태" width="80%"}

## 4단계

![Extract: D. Update: F=8/D, G=8/D. Selected: AD11.](../figures/08-graphs/13-prim-trace-step4.svg){fig-alt="D가 추출되고 AD edge가 추가된 상태" width="80%"}

## 5단계

![Extract: F. Update: G=7/F. Selected: DF8.](../figures/08-graphs/13-prim-trace-step5.svg){fig-alt="F가 추출되고 DF edge가 추가된 상태" width="80%"}

## 6단계

![Extract: G. Update: done. Selected: FG7.](../figures/08-graphs/13-prim-trace-step6.svg){fig-alt="G가 추출되어 MST가 완성된 상태" width="80%"}
:::
...
![accept CE. Selected: 1 edges. Weight: 5.](../figures/08-graphs/15-kruskal-trace-step1.svg){fig-alt="CE edge가 선택된 상태" width="80%"}

## 2단계

![accept FG. Selected: 2 edges. Weight: 12.](../figures/08-graphs/15-kruskal-trace-step2.svg){fig-alt="FG edge가 추가된 상태" width="80%"}

## 3단계

![accept AB. Selected: 3 edges. Weight: 20.](../figures/08-graphs/15-kruskal-trace-step3.svg){fig-alt="AB edge가 추가된 상태" width="80%"}

## 4단계

![accept DF. Selected: 4 edges. Weight: 28.](../figures/08-graphs/15-kruskal-trace-step4.svg){fig-alt="DF edge가 추가된 상태" width="80%"}

## 5단계

![reject DG. Selected: 4 edges. Weight: 28.](../figures/08-graphs/15-kruskal-trace-step5.svg){fig-alt="DG edge가 이미 D-F-G로 연결되어 있어 reject되는 상태" width="80%"}

## 6단계

![accept AC. Selected: 5 edges. Weight: 37.](../figures/08-graphs/15-kruskal-trace-step6.svg){fig-alt="AC edge가 추가된 상태" width="80%"}

## 7단계

![reject BE; accept AD. Selected: 6 edges. Weight: 48.](../figures/08-graphs/15-kruskal-trace-step7.svg){fig-alt="BE는 reject되고 AD가 추가되어 MST가 완성된 상태" width="80%"}
:::
...
![$A$-$B$-$C$-$D$ 사각형, 모든 edge weight $1$ — cycle의 어느 edge를 빼도 total 3인 MST다.](../figures/08-graphs/16-mst-tie-square.svg){fig-alt="A,B,C,D가 정사각형으로 연결되고 모든 edge의 weight가 1로 표시된 다이어그램" width="40%"}
...
![$dist[u]=4,\ w(u,v)=-2$. Candidate $=2$, old $dist[v]=7$. compare.](../figures/08-graphs/17-relaxation-animation-step1.svg){fig-alt="dist[u]=4, w(u,v)=-2인 상태에서 후보값 2와 기존 dist[v]=7을 비교하는 상태" width="60%"}

## 2단계

![update $dist[v]=2,\ parent[v]=u$.](../figures/08-graphs/17-relaxation-animation-step2.svg){fig-alt="dist[v]가 2로 갱신되고 parent가 u로 설정되는 상태" width="60%"}

## 3단계

![한 번의 Relax가 final을 보장하지는 않는다.](../figures/08-graphs/17-relaxation-animation-step3.svg){fig-alt="한 번의 relax만으로는 최종값이 아닐 수 있음을 알리는 상태" width="60%"}
:::
...
::: {.callout-tip title="5장(동적 계획법)과의 연결"}
위상 순서로 한 번씩 relax하는 이 evaluation order는 **5장의 DP evaluation order와 같은
구조**다 — 위상 순서는 "모든 의존 state가 먼저 계산되어 있는 순서"라는 DP의 요구사항을 그대로
만족시킨다. Relax 자체도 $dist[v]\gets\min(dist[v],\,dist[u]+w(u,v))$ 형태로, 5장에서 본
"현재 state를 더 작은 state들로부터 갱신"하는 DP transition과 같은 모양이다.
:::
...
![after $s$: $d[a]=3,d[b]=2$.](../figures/08-graphs/19-dag-relaxation-trace-step1.svg){fig-alt="s를 처리한 뒤 a와 b의 거리가 갱신된 상태" width="60%"}

## 2단계

![after $a$: $d[c]=-1$.](../figures/08-graphs/19-dag-relaxation-trace-step2.svg){fig-alt="a를 처리한 뒤 c의 거리가 -1로 갱신된 상태" width="60%"}

## 3단계

![after $b$: candidate 3, no update. answer $d[c]=-1$, path $s\to a\to c$.](../figures/08-graphs/19-dag-relaxation-trace-step3.svg){fig-alt="b를 처리했지만 후보값 3이 기존 -1보다 크므로 갱신되지 않는 최종 상태" width="60%"}
:::
...
::: {.callout-tip title="3장의 heap이 여기서도 다시 쓰인다"}
Prim에서 다시 쓴 3장의 min-heap이 Dijkstra의 ExtractMin/Insert 자료구조로 그대로 이어진다 —
...
![Extract: S(0). Update: A=4, B=2. Finalized: S.](../figures/08-graphs/20-dijkstra-trace-step1.svg){fig-alt="S가 finalize되고 A,B의 거리가 초기 갱신된 상태" width="80%"}

## 2단계

![Extract: B(2). Update: A=3, C=10, D=12. Finalized: S,B.](../figures/08-graphs/20-dijkstra-trace-step2.svg){fig-alt="B가 finalize되고 A가 3으로 더 짧게 갱신된 상태" width="80%"}

## 3단계

![Extract: A(3). Update: C=8. Finalized: S,B,A.](../figures/08-graphs/20-dijkstra-trace-step3.svg){fig-alt="A가 finalize되고 C가 8로 갱신된 상태" width="80%"}

## 4단계

![Extract: C(8). Update: D=10, E=14. Finalized: S,B,A,C.](../figures/08-graphs/20-dijkstra-trace-step4.svg){fig-alt="C가 finalize되고 D,E가 갱신된 상태" width="80%"}

## 5단계

![Extract: D(10). Update: E=13. Finalized: S,B,A,C,D.](../figures/08-graphs/20-dijkstra-trace-step5.svg){fig-alt="D가 finalize되고 E가 13으로 더 짧게 갱신된 상태" width="80%"}

## 6단계

![Extract: E(13). Update: done. Finalized: all.](../figures/08-graphs/20-dijkstra-trace-step6.svg){fig-alt="모든 vertex가 finalize된 최종 상태" width="80%"}
:::
...
![$s\to a$(2), $s\to b$(5), $b\to a$(-10) — $a$는 거리 2로 먼저 finalize되지만 실제 최단은 $s\to b\to a=-5$다.](../figures/08-graphs/21-dijkstra-negative-counterexample.svg){fig-alt="s에서 a로 직접(2), s에서 b로(5), b에서 a로 음수(-10) edge가 그려진 다이어그램" width="55%"}
...
![$|V|-1$ pass 후 한 번 더 relax가 가능하다 — $a\to b$를 조사한다.](../figures/08-graphs/22-bellman-ford-negative-cycle-step1.svg){fig-alt="V-1 패스 이후에도 relax가 가능하다는 안내와 a에서 b로 가는 edge를 조사하는 상태" width="60%"}

## 2단계

![reachable negative cycle detected.](../figures/08-graphs/22-bellman-ford-negative-cycle-step2.svg){fig-alt="negative cycle이 검출되었다는 붉은 강조 상태" width="60%"}
:::

::: {.callout-tip title="Bellman-Ford의 DP 해석 — 5장(동적 계획법)의 회수"}
$$
...
$$
이 recurrence는 **5장에서 배운 DP 다섯 요소**로 그대로 읽힌다 — state는 $(k,v)$("최대
...
- MST(Prim/Kruskal)는 3장의 heap을 다시 쓰고, cut property의 exchange argument로 정당화된다
  — MST는 shortest-path tree가 아니다.
- Dijkstra는 nonnegative weight를 전제하고, Bellman-Ford는 negative edge를 허용하며 5장의
  DP recurrence로 재해석된다 — DAG에서는 위상 순서로 relax하면 negative edge도 안전하다.
...
property의 exchange argument로 정당화되며, Prim(vertex key + 3장의 heap)과 Kruskal(edge
...
unweighted, DAG SP는 위상 순서 relax(5장의 DP evaluation order와 같은 구조), Dijkstra는
nonnegative weight(3장의 heap을 다시 쓴다), Bellman-Ford는 negative edge를 허용하고 $|V|-1$
pass 뒤 한 번 더 relax해 negative cycle을 검출하며 5장의 DP recurrence로 재해석된다.
