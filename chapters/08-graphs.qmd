...
![Row 0: $0,1,1,0$ — nonzero column을 찾는다.](../figures/08-graphs/03-matrix-to-list-step1.svg){fig-alt="matrix에서 row 0의 값을 나열하고 nonzero column을 찾는 설명 콜아웃" width="30%"}

## 2단계

![$\operatorname{Adj}[0]=[1,2]$ — row 1, 2, 3도 같은 방식으로 반복한다.](../figures/08-graphs/03-matrix-to-list-step2.svg){fig-alt="Adj[0]이 [1,2]로 채워지고 나머지 row도 반복한다는 콜아웃" width="20%"}

## 3단계

![matrix 스캔은 $\Theta(V^2)$, list 저장은 $\Theta(V+E)$.](../figures/08-graphs/03-matrix-to-list-step3.svg){fig-alt="matrix scan 비용과 list storage 비용을 비교하는 콜아웃" width="30%"}
:::
