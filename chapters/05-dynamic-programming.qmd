...
재귀 장에서 이미 본 Fibonacci의 순진한 재귀는 정확하지만, 같은 부분 문제를 몇 번이고 다시
...
![학습 로드맵: 재귀에서 출발해 중복 부분 문제를 인식하고, Memoization·Tabulation을 거쳐 대표 문제(Matrix Path, LCS, Maximum Subarray)로 이어지는 흐름.](../figures/05-dynamic-programming/01-roadmap.svg){fig-alt="재귀에서 시작해 중복 부분 문제 인식, Memoization과 Tabulation, 대표 DP 문제로 이어지는 트리 형태의 로드맵 다이어그램" width="70%"}

![Recurrence(점화식) · Memoization(하향식 저장) · Tabulation(상향식 저장)의 관계: 셋 다 같은 재귀 관계식을 표현하되, 계산 순서와 저장 방식만 다르다.](../figures/05-dynamic-programming/02-recurrence-memo-tab.svg){fig-alt="Recurrence 노드에서 Memoization과 Tabulation 두 갈래로 뻗어나가는 관계도, 두 방식 모두 같은 재귀 관계식으로 되돌아옴을 화살표로 표시" width="65%"}
...
이 책에서 $F(0)=0$, $F(1)=1$, $F(n)=F(n-1)+F(n-2)$ $(n\ge2)$로 고정한다. (재귀 장의 Fibonacci
호출 트리도 같은 컨벤션을 썼다.)
:::
...
![$F(6)$의 순진한 재귀 호출 트리 — $F(3)$과 $F(2)$가 서로 다른 호출·activation record로 반복된다(재귀 장의 $F(5)$ 호출 트리와 같은 현상, 여기서는 하나 더 큰 $n$으로 반복 정도가 커짐을 보여준다).](../figures/05-dynamic-programming/03-fib-call-tree.svg){fig-alt="F(6)을 루트로 F(5), F(4)가 자식이 되고 그 아래로 F(4), F(3), F(2), F(1)이 반복해서 나타나는 이진 트리, 반복되는 노드가 색으로 강조됨" width="70%"}

중복 호출을 세어 보면 $F(3)$은 3번, $F(2)$는 5번, $F(1)$은 8번, $F(0)$은 5번 다시 계산된다 —
...
![호출 전 base states만 초기화된 상태 — $F(6)$은 아직 cache miss다.](../figures/05-dynamic-programming/04-memo-trace-step1.svg){fig-alt="0번과 1번 칸만 채워지고 나머지는 물음표로 표시된 캐시 상태" width="60%"}

## 2단계

![$F(1)$, $F(0)$은 cache hit, $F(2)$가 새로 저장된다.](../figures/05-dynamic-programming/04-memo-trace-step2.svg){fig-alt="0,1,2번 칸이 채워지고 나머지는 물음표인 캐시 상태" width="60%"}

## 3단계

![재귀가 내려갔다 올라오며 각 non-base state가 정확히 한 번씩 채워진다.](../figures/05-dynamic-programming/04-memo-trace-step3.svg){fig-alt="0부터 4번 칸까지 채워지고 5,6번은 물음표인 캐시 상태" width="60%"}

## 4단계

![answer $F(6)=8$ — $F(4)$가 다시 요청되어도 cache hit이라 재계산하지 않는다.](../figures/05-dynamic-programming/04-memo-trace-step4.svg){fig-alt="0번부터 6번까지 모두 채워지고 6번 칸이 정답으로 강조된 캐시 상태" width="60%"}
:::
...
![Base case $dp[0]=0,\ dp[1]=1$만 채워진 상태.](../figures/05-dynamic-programming/05-bottomup-trace-step1.svg){fig-alt="0,1번 칸만 채워진 테이블 상태" width="60%"}

## 2단계

![왼쪽에서 오른쪽으로 진행하며 각 칸이 두 이전 칸에만 의존하는 순서.](../figures/05-dynamic-programming/05-bottomup-trace-step2.svg){fig-alt="0부터 3번 칸까지 채워진 테이블 상태" width="60%"}

## 3단계

![answer $F(6)=8$, 시간 $\Theta(n)$.](../figures/05-dynamic-programming/05-bottomup-trace-step3.svg){fig-alt="0번부터 6번까지 모두 채워지고 6번 칸이 정답으로 강조된 테이블 상태" width="60%"}
:::
...
::: {.callout-note collapse="true" title="Memoization vs Bottom-Up"}
...
탐욕(greedy)이 왜 위험한지 같은 $3\times3$ 행렬로 확인해 보자. 탐욕은 현재 칸에서 보이는
다음 칸들의 값만 비교해 그 자리에서 하나를 확정하므로, 그 다음에 얼마나 큰 값이 기다리고
있는지 알 수 없다 — Matrix Path의 최적해는 도착점까지 전체 경로의 합을 최소화해야 하므로,
지금 당장 작아 보이는 선택이 나중에 더 큰 값을 강제로 지나게 만들 수 있다.

![매 칸에서 더 작은 다음 칸(오른쪽 우선)만 보고 이동하면 첫 row를 따라가다 100을 지나게 되어, 합이 $1+2+100+1+1=105$로 매우 나쁘다.](../figures/05-dynamic-programming/06-greedy-counterexample-local.svg){fig-alt="3x3 격자에서 지역적으로 작은 값을 따라가 합 105가 되는 경로가 강조된 다이어그램" width="20%"}

![전역 최적 경로는 첫 column을 따라 내려간 뒤 마지막 row를 따라가 두 100을 모두 피하고, 합 $1+3+1+1+1=7$을 달성한다.](../figures/05-dynamic-programming/07-greedy-counterexample-optimal.svg){fig-alt="같은 3x3 격자에서 전역 최적 경로(합 7)가 강조된 다이어그램" width="20%"}
...
수학의 $dp[i][j]$, $(1,1)\to(n,m)$은 1-based다. C/Java/Python 배열 인덱스는 0-based이므로
같은 칸이 코드에서는 `dp[i-1][j-1]`, 도착 칸은 `dp[n-1][m-1]`이 된다. 아래 Checkpoint의
$dp[1][3]$ 같은 표기도 이 1-based 좌표를 그대로 쓴 것이다 — 코드 인덱스와 혼동하지 않는다.
:::
...
![$dp[3][3]$ 계산의 확대: 위쪽 25와 왼쪽 31 중 더 작은 25를 골라(강조 표시) $M[3][3]=3$을 더해 28이 된다.](../figures/05-dynamic-programming/11-matrix-representative-cell.svg){fig-alt="dp[3][3] 칸이 위쪽 25, 왼쪽 31 두 이전 칸으로부터 화살표를 받는 확대 다이어그램. 선택된 25는 결과 28과 같은 주황색으로 강조되어 있다" width="19%"}
...
![match 시 대각선 값에 1을 더한다.](../figures/05-dynamic-programming/13-lcs-case1.svg){fig-alt="대각선 위 칸에서 화살표가 현재 칸으로 향하고 +1이 표시된 다이어그램" width="15%"}
...
![$(7,6)$에서 시작해 대각선·위·왼쪽을 오가며 뒤로 거슬러 올라간 뒤, 선택된 문자를 역순으로 뒤집으면 "BCBA"가 된다.](../figures/05-dynamic-programming/15-lcs-backtrack-trace.svg){fig-alt="LCS 표 위에 (6,6)=A, (4,5)=B, (3,3)=C, (2,1)=B 네 칸이 강조되고 화살표로 역추적 경로가 표시된 다이어그램, 오른쪽에 선택된 문자 A,B,C,B를 역순으로 뒤집어 BCBA를 얻는 과정이 텍스트로 표시됨" width="80%"}
...
![DP를 중심으로 State·Transition·Order·Answer 네 요소가 뻗어나가고, 다시 "state 수 × transition 비용" 복잡도로 수렴하는 개념 지도.](../figures/05-dynamic-programming/16-concept-map.svg){fig-alt="Dynamic Programming을 중심 노드로 State, Transition, Order, Answer 네 갈래가 뻗어나가고 이들이 다시 복잡도 노드로 모이는 다이어그램" width="55%"}
...
부분 문제가 겹칠 때만 저장이 이득이 된다 — 이 구분이 정렬 장의 분할정복(Master Theorem으로
