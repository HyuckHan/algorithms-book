...
![매 칸에서 더 작은 다음 칸(오른쪽 우선)만 보고 이동하면 첫 row를 따라가다 두 개의 100을 모두 지나게 되어, 합이 $1+2+100+1+1=105$로 매우 나쁘다.](../figures/05-dynamic-programming/06-greedy-counterexample-local.svg){fig-alt="3x3 격자에서 지역적으로 작은 값을 따라가 합 105가 되는 경로가 강조된 다이어그램" width="20%"}
...
![$(7,6)$에서 시작해 대각선·위·왼쪽을 오가며 뒤로 거슬러 올라간 뒤, 선택된 문자를 역순으로 뒤집으면 "BCBA"가 된다.](../figures/05-dynamic-programming/15-lcs-backtrack-trace.svg){fig-alt="LCS 표 위에 (8,8)=A, (6,7)=B, (5,5)=C, (4,3)=B 네 칸이 강조되고 화살표로 역추적 경로가 표시된 다이어그램, 오른쪽에 선택된 문자 A,B,C,B를 역순으로 뒤집어 BCBA를 얻는 과정이 텍스트로 표시됨" width="80%"}
