...
![학습 로드맵: 재귀에서 출발해 중복 부분 문제를 인식하고, Memoization·Tabulation을 거쳐 대표 문제(Matrix Path, LCS, Maximum Subarray)로 이어지는 흐름.](../figures/05-dynamic-programming/01-roadmap.svg){fig-alt="재귀에서 시작해 중복 부분 문제 인식, Memoization과 Tabulation, 대표 DP 문제로 이어지는 트리 형태의 로드맵 다이어그램" width="90%"}
...
![매 칸에서 더 작은 다음 칸(오른쪽 우선)만 보고 이동하면 첫 row를 따라가다 두 개의 100을 모두 지나게 되어, 합이 $1+2+100+1+1=105$로 매우 나쁘다.](../figures/05-dynamic-programming/06-greedy-counterexample-local.svg){fig-alt="3x3 격자에서 지역적으로 작은 값을 따라가 합 105가 되는 경로가 강조된 다이어그램" width="45%"}

![전역 최적 경로는 첫 column을 따라 내려간 뒤 마지막 row를 따라가 두 100을 모두 피하고, 합 $1+3+1+1+1=7$을 달성한다.](../figures/05-dynamic-programming/07-greedy-counterexample-optimal.svg){fig-alt="같은 3x3 격자에서 전역 최적 경로(합 7)가 강조된 다이어그램" width="45%"}
