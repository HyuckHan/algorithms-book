...
![$sum(4)$ 호출: 첫 frame이 쌓인다.](../figures/02-recursion/01-call-stack-push-step1.svg){fig-alt="sum(4) frame 하나가 강조되어 쌓인 상태" width="30%"}

## 2단계

![$sum(3)$ 호출: frame이 그 위에 쌓인다.](../figures/02-recursion/01-call-stack-push-step2.svg){fig-alt="sum(4) 위에 sum(3) frame이 강조되어 쌓인 상태" width="30%"}

## 3단계

![$sum(2)$ 호출: frame이 그 위에 쌓인다.](../figures/02-recursion/01-call-stack-push-step3.svg){fig-alt="sum(4), sum(3) 위에 sum(2) frame이 강조되어 쌓인 상태" width="30%"}

## 4단계

![$sum(1)$ 호출: frame이 그 위에 쌓인다.](../figures/02-recursion/01-call-stack-push-step4.svg){fig-alt="sum(4), sum(3), sum(2) 위에 sum(1) frame이 강조되어 쌓인 상태" width="30%"}

## 5단계

![$sum(0)$ 호출: base case 도달, 다섯 frame 전부 쌓인다.](../figures/02-recursion/01-call-stack-push-step5.svg){fig-alt="sum(4)부터 sum(0)까지 다섯 개의 frame이 쌓이고 맨 위 sum(0)이 강조된 최종 상태" width="30%"}
:::
...
![가장 큰 원판을 L→R로 직접 이동(4회째). 이제 M의 작은 두 원판을 R로 옮기면 된다.](../figures/02-recursion/08-hanoi-n3-states-step3.svg){fig-alt="L은 비고 M에 작은 두 원판, R에 가장 큰 원판이 놓인 상태" width="70%"}

## 전체 완료

![큰 원판 이동 후 $H(2,M,R,L)$까지 완료 — 총 7회.](../figures/02-recursion/08-hanoi-n3-states-step4.svg){fig-alt="세 원판이 모두 R로 옮겨진 최종 상태" width="70%"}
:::
...
그대로 남겼다. 이웃 순서(위·아래·왼쪽·오른쪽)는 고정되어 있고, 아래 트레이스 그림은 그 순서를
그대로 따른다 — 아래 인쇄된 실행 결과의 경로와 트레이스 그림 마지막 단계의 경로는 같은 순서
(`(0,0)->(0,1)->(1,1)->(2,1)->(2,2)->(3,2)->(4,2)->(4,3)->(4,4)->(5,4)->(5,5)`)다.
...
![S에서 후보 경로를 따라 전진.](../figures/02-recursion/10-maze-trace-step1.svg){fig-alt="6x6 격자에서 시작점 S부터 세 칸이 후보 경로로 표시된 상태" width="45%"}

## 2단계

![막혀서 dead end.](../figures/02-recursion/10-maze-trace-step2.svg){fig-alt="후보 경로 끝에 막다른 칸이 dead로 표시된 상태" width="45%"}

## 3단계

![되돌아가 아래로 내려간 뒤 오른쪽 통로로 꺾어 전진.](../figures/02-recursion/10-maze-trace-step3.svg){fig-alt="경로가 아래쪽으로 확장된 뒤 오른쪽으로 꺾여 후보 경로 한 줄기로 이어진 상태" width="45%"}

## 4단계

![위쪽 통로로 전진을 시도하는 중.](../figures/02-recursion/10-maze-trace-step4.svg){fig-alt="후보 경로가 위쪽 통로를 따라 두 칸 더 확장된 상태" width="45%"}

## 5단계

![더 나아갈 수 없어 되돌아가며 지나온 칸을 dead로 표시.](../figures/02-recursion/10-maze-trace-step5.svg){fig-alt="방금 올라간 두 칸이 dead로 표시되고 경로가 되돌아간 상태" width="45%"}

## 6단계

![출구 E까지 경로 완성.](../figures/02-recursion/10-maze-trace-step6.svg){fig-alt="시작 S부터 출구 E까지 이어진 전체 경로가 표시된 최종 상태" width="45%"}
:::
