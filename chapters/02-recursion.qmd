...
![$sum(4)$ 호출: 첫 frame이 쌓인다.](../figures/02-recursion/01-call-stack-push-step1.svg){fig-alt="sum(4) frame 하나가 강조되어 쌓인 1단계 상태" width="30%"}
...
![되돌아가 다른 방향(아래)으로 전진.](../figures/02-recursion/10-maze-trace-step3.svg){fig-alt="경로가 아래쪽으로 확장되어 후보 경로 한 줄기로 이어진 상태" width="45%"}

## 4단계

![다시 막혀서 dead end, 되돌아간다.](../figures/02-recursion/10-maze-trace-step4.svg){fig-alt="방금 내려간 두 칸이 dead로 표시되고 경로가 되돌아간 상태" width="45%"}

## 5단계

![되돌아가 또 다른 방향으로 재시도.](../figures/02-recursion/10-maze-trace-step5.svg){fig-alt="이전과 다른 방향으로 경로가 확장된 상태" width="45%"}
