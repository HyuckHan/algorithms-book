...
![$sum(4)$ 호출: 첫 frame이 쌓인다.](../figures/02-recursion/01-call-stack-push-step1.svg){fig-alt="sum(4) frame 하나만 쌓인 1단계 상태" width="30%"}

## 2단계

![$sum(4)\to sum(3)\to sum(2)$: 재귀 호출이 반복되며 frame이 쌓인다.](../figures/02-recursion/01-call-stack-push-step2.svg){fig-alt="sum(4), sum(3), sum(2) 세 개의 frame이 아래에서 위로 쌓인 상태" width="30%"}

## 3단계

![$sum(1), sum(0)$까지 도달: base case에서 멈춘다.](../figures/02-recursion/01-call-stack-push-step3.svg){fig-alt="sum(4)부터 sum(0)까지 다섯 개의 frame이 쌓이고 맨 위 sum(0)이 강조된 최종 상태" width="30%"}
:::
