...
![공통 첫 partition: pivot 15가 index 4에 위치하고 p=1, r=10, i∈{2,7}, q=4, k=4다.](../figures/04-selection/06-common-first-partition.svg){fig-alt="8,11,3,15,31,48,20,29,65,73 배열에서 15가 pivot으로 강조되고 p=1,r=10,i∈{2,7},q=4,k=4라는 콜아웃이 붙은 다이어그램" width="70%"}
...
![25개 원소를 5개씩 5개 group(G1–G5)으로 묶는다.](../figures/04-selection/10-groups-of-five.svg){fig-alt="22,4,17,9,31부터 20,3,29,7,24까지 5행 5열로 배치되고 각 행이 G1부터 G5까지 라벨된 group box로 묶인 다이어그램" width="55%"}
...
![Group Sort → Median → Median of Medians: 각 group을 정렬해 3번째 값을 median으로 뽑고, 다섯 median의 median M=16을 원본의 pivot으로 쓴다.](../figures/04-selection/11-median-of-medians-trace-step1.svg){fig-alt="G1..G5 각 group의 원소들이 나열되고 가운데 열(3번째 값)이 강조된 1단계 상태" width="60%"}
...
{{< include ../figures/04-selection/pseudocode-deterministic-select-pivot.qmd >}}

$p+i-1$은 유효한 absolute index이며, $M$은 원본 $A[p..r]$에 실제로 존재하는 값이다.

{{< include ../figures/04-selection/pseudocode-deterministic-select-partition.qmd >}}

$$A[p..lt-1]<M,\quad A[lt..gt]=M,\quad A[gt+1..r]>M$$

이제 group median 수집이 원본 원소를 양쪽에서 얼마나 제거하는지 계산할 차례다.

### Pivot 보장: 왜 양쪽에서 일정 비율이 제거되는가

medians를 정렬했을 때 $M$은 lower median이므로, 적어도 절반은 $M$ 이상이고 대칭적으로 적어도 절반은 $M$ 이하다.

![다섯 median 중 절반(12,14)은 M=16 이하, 절반(17,20)은 M 이상이다.](../figures/04-selection/12-median-half-guarantee.svg){fig-alt="12,14가 guaranteed less로, M=16이 중앙에, 17,20이 guaranteed greater로 표시된 다이어그램" width="55%"}

full group을 정렬했을 때 median이 $M$ 이상이면, 그 median과 오른쪽 두 값(더 큰 값들)까지 총 **3개**가 $M$ 이상이다.

![median이 M=16 이상인 group에서는 median과 그 오른쪽 두 값, 총 3개가 M 이상이다.](../figures/04-selection/13-group-three-guarantee.svg){fig-alt="unknown 값 두 개, median 16, guaranteed greater 값 두 개가 나열되고 오른쪽 세 개가 M 이상이라는 중괄호 표시가 붙은 다이어그램" width="55%"}
...
![Selection 개념 전체: Sort then Select(Θ(n log n)), Quickselect(Fixed-pivot worst Θ(n²), RandomizedSelect expected Θ(n)/worst Θ(n²)), DeterministicSelect(Median of Medians, worst Θ(n))로 정리한다.](../figures/04-selection/14-concept-map.svg){fig-alt="Selection을 중심으로 Sort then Select, Quickselect, DeterministicSelect 세 갈래로 나뉘고 각각 시간 복잡도로 이어지는 개념 지도" width="80%"}
