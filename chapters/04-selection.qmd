...
![현재 last element 3 외에 다른 pivot을 고르지 않는다.](../figures/04-selection/07-fixed-pivot-trace-step1.svg){fig-alt="8,11,3이 active, 15부터 73까지가 discard로 표시되고 pivot 3이 강조된 1단계 상태" width="55%"}
...
![원래 배열의 2nd order-statistic value는 8이다.](../figures/04-selection/07-fixed-pivot-trace-step5.svg){fig-alt="8이 target으로 강조되고 나머지가 모두 discard된 마지막 상태, answer=8" width="55%"}
:::
...
![선택한 pivot index와 value를 먼저 명시한다: s=8, pivot=29.](../figures/04-selection/08-randomized-trace-step1.svg){fig-alt="31,48,20,29,65,73이 active인 상태에서 p=5,r=10,i=3, 무작위 draw s=8, pivot 29라는 콜아웃이 붙은 1단계 상태" width="65%"}
...
![원래 배열의 7th order-statistic value는 31이다.](../figures/04-selection/08-randomized-trace-step7.svg){fig-alt="31이 target으로 강조되고 73,65,48이 discard된 마지막 상태, answer=31" width="65%"}
:::
...
![각 group을 정렬한다.](../figures/04-selection/11-median-of-medians-trace-step1.svg){fig-alt="G1..G5 각 group의 원소들이 나열되고 가운데 열(3번째 값)이 강조된 1단계 상태" width="55%"}
...
![M=16이 원본 배열의 pivot이 된다.](../figures/04-selection/11-median-of-medians-trace-step5.svg){fig-alt="다섯 median 12,14,16,17,20 중 16이 M으로 강조되고 원본의 pivot이라는 콜아웃이 붙은 마지막 상태" width="60%"}
:::
