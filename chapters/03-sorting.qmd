...
세 알고리즘 모두 Java/C 참조 구현과 같은 예제 입력 `[29, 10, 14, 37, 13, 5, 21, 8]`을 사용한다(Java/C 참조 코드는 이 강의에서 표준 라이브러리 API·comparator 안전성에 집중하므로, from-scratch 구현은 Python으로만 제공한다 — Part E 참조).

```{.python include="../code/03-sorting/python/sorting.py"}
```

**실행 결과** (`python3 code/03-sorting/python/sorting.py`, 실제 실행 캡처):

```{.default include="../figures/03-sorting/out-sorting.txt"}
```
...
`mid = low + (high - low) / 2`처럼 **뺄셈 뒤 더하는** 형태를 써야 한다. `mid = (low + high) / 2`는 `low + high`가 정수 overflow를 일으킬 수 있어 위험하다(PER_LECTURE_NOTES L03 교정 항목).
:::
...
비교 정렬의 하한은 $O(n\log n)$이 **아니라 $\Omega(n\log n)$**이다(PER_LECTURE_NOTES L03 교정 항목 — $O$는 상한이므로 "적어도 이만큼 걸린다"는 하한 주장에는 맞지 않는다).
:::
...
"$n/2$개의 node × 각 $O(\log n)$" 논증은 **느슨한 상한** $O(n\log n)$만 준다(PER_LECTURE_NOTES L03 교정 항목). 대부분의 node는 아래쪽에 있어 높이가 0 또는 1이므로 모두가 root 높이만큼 내려가지 않는다.
...
`Comparator`를 별도 클래스로 `extends`하는 것은 오래된 오개념이다(PER_LECTURE_NOTES L03 교정 항목). `Comparator<T>`는 **functional interface**이므로 람다나 `Comparator.comparingInt(...).thenComparing(...)` 체이닝으로 작성한다.
:::
...
정수 comparator에서 `return a - b;`는 두 값이 충분히 멀면 **overflow**로 잘못된 부호를 반환할 수 있다(PER_LECTURE_NOTES L03 교정 항목). `(a > b) - (a < b)`처럼 **relational comparison**을 쓰거나 Java Comparator 방식을 따라야 한다.
:::
...
::: {.panel-tabset}
## Python
```{.python include="../code/03-sorting/python/comparator_demo.py"}
```
## Java
...
::: {.panel-tabset}
## Python
```{.default include="../figures/03-sorting/out-comparator_demo.txt"}
```
## Java
```{.default include="../figures/03-sorting/out-java.txt"}
```
## C
```{.default include="../figures/03-sorting/out-c.txt"}
```
:::
