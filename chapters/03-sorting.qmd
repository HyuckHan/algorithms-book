...
![Stability: 왼쪽(원본)에서 key가 같은 A:90과 C:90의 상대 순서가, 오른쪽(정렬 후)에서도 보존된다.](../figures/03-sorting/02-stability-demo.svg){fig-alt="90,70,90,80 순서의 레코드가 정렬 후 70,80,90,90이 되며 두 개의 90 레코드(A와 C)의 상대 순서가 보존됨을 화살표로 표시한 다이어그램" width="50%"}
...
Selection Sort 5단계 추적. 주황은 이번 pass에서 선택된 최댓값, 회색은 확정된 sorted suffix다.

::: {.panel-tabset}
## 시작

![최댓값 37을 찾는다: 29, 10, 14, 37, 13.](../figures/03-sorting/03-selection-trace-step1.svg){fig-alt="5칸 배열 29,10,14,37,13에서 37이 주황으로 강조된 상태" width="40%"}

## Pass 1

![37을 맨 뒤와 swap: 29, 10, 14, 13, 37.](../figures/03-sorting/03-selection-trace-step2.svg){fig-alt="37이 맨 뒤로 이동해 회색으로 확정되고 나머지 29,10,14,13에서 다음 최댓값을 찾는 상태" width="40%"}

## Pass 2

![나머지에서 최댓값 29를 찾아 13과 swap: 13, 10, 14, 29, 37.](../figures/03-sorting/03-selection-trace-step3.svg){fig-alt="29가 회색으로 확정되고 나머지 13,10,14에서 다음 최댓값을 찾는 상태" width="40%"}

## Pass 3

![14는 이미 제자리라 swap 없이 확정: 13, 10, 14, 29, 37.](../figures/03-sorting/03-selection-trace-step4.svg){fig-alt="14가 swap 없이 회색으로 확정되고 나머지 13,10에서 다음 최댓값을 찾는 상태" width="40%"}

## Pass 4

![13과 10을 swap하여 정렬 완료: 10, 13, 14, 29, 37.](../figures/03-sorting/03-selection-trace-step5.svg){fig-alt="10,13,14,29,37로 완전히 정렬되어 모두 회색으로 표시된 상태" width="40%"}
:::
...
![29와 10 비교: 역전이므로 swap.](../figures/03-sorting/04-bubble-trace-step1.svg){fig-alt="29,10,14,37,13 배열에서 29와 10이 강조된 상태" width="40%"}

## 2단계

![29와 14 비교: swap.](../figures/03-sorting/04-bubble-trace-step2.svg){fig-alt="10,29,14,37,13 배열에서 29와 14가 강조된 상태" width="40%"}

## 3단계

![29와 37 비교: 이미 순서대로라 비교만(swap 없음).](../figures/03-sorting/04-bubble-trace-step3.svg){fig-alt="10,14,29,37,13 배열에서 29와 37이 빗금 표시로 강조되어 비교만 하고 swap하지 않은 상태" width="40%"}

## 4단계

![37과 13 비교: swap.](../figures/03-sorting/04-bubble-trace-step4.svg){fig-alt="10,14,29,37,13 배열에서 37과 13이 강조된 상태" width="40%"}

## 5단계

![첫 pass 종료: 최댓값 37 확정.](../figures/03-sorting/04-bubble-trace-step5.svg){fig-alt="10,14,29,13,37 배열에서 37이 오른쪽 끝에 확정된 상태" width="40%"}
:::
...
![새 key를 꺼내고 큰 원소를 한 칸씩 shift하여 생긴 hole에 넣는다.](../figures/03-sorting/05-insertion-cards.svg){fig-alt="10,14,29,13 네 장의 카드 위로 13에서 10과 14 사이 gap까지 호를 그리는 삽입 화살표가 표시된 다이어그램" width="35%"}

::: {.panel-tabset}
## 1단계

![key를 꺼내 hole 생성.](../figures/03-sorting/05-insertion-trace-step1.svg){fig-alt="10,14,29,13,37 배열에서 index 4에 hole이 생기고 key=13인 상태" width="40%"}

## 2단계

![29 shift.](../figures/03-sorting/05-insertion-trace-step2.svg){fig-alt="29가 오른쪽으로 shift되어 hole이 index 3으로 옮겨진 상태" width="40%"}

## 3단계

![14 shift.](../figures/03-sorting/05-insertion-trace-step3.svg){fig-alt="14가 오른쪽으로 shift되어 hole이 index 2로 옮겨진 상태" width="40%"}

## 4단계

![13 insert.](../figures/03-sorting/05-insertion-trace-step4.svg){fig-alt="13이 hole 자리에 삽입되어 10,13,14,29,37이 된 상태" width="40%"}

## 5단계

![다음 반복 전에 정렬된 prefix 확인.](../figures/03-sorting/05-insertion-trace-step5.svg){fig-alt="10,13,14,29,37로 정렬된 prefix가 확인된 상태" width="40%"}
:::
...
1. $[2_b,2_a,4,5]$ — equal record가 역전될 수 있다(unstable). 
2. $[2_a,4,2_b,5]$. 
3. 5와 4만 shift하고 $2_a$는 움직이지 않는다.
:::
...
두 정렬된 배열 L=[10,14,29,37], R=[5,8,13,21]을 두 pointer로 병합한다.

::: {.panel-tabset}
## 1단계

![out=[]에서 R의 5 선택.](../figures/03-sorting/07-merge-pointers-step1.svg){fig-alt="L과 R 두 배열 위에서 R의 5가 강조되어 out의 첫 칸에 놓이는 상태" width="55%"}

## 2단계

![out=[5]에서 R의 8 선택.](../figures/03-sorting/07-merge-pointers-step2.svg){fig-alt="out에 5가 채워지고 R의 8이 강조되어 다음으로 선택되는 상태" width="55%"}

## 3단계

![out=[5,8]에서 L의 10 선택.](../figures/03-sorting/07-merge-pointers-step3.svg){fig-alt="out에 5,8이 채워지고 L의 10이 강조되어 다음으로 선택되는 상태" width="55%"}

## 4단계

![out=[5,8,10]에서 R의 13 선택.](../figures/03-sorting/07-merge-pointers-step4.svg){fig-alt="out에 5,8,10이 채워지고 R의 13이 강조되어 다음으로 선택되는 상태" width="55%"}

## 5단계

![out=[5,8,10,13]에서 L의 14 선택.](../figures/03-sorting/07-merge-pointers-step5.svg){fig-alt="out에 5,8,10,13이 채워지고 L의 14가 강조되어 다음으로 선택되는 상태" width="55%"}

## 6단계

![out=[5,8,10,13,14]에서 R의 21 선택(R 소진).](../figures/03-sorting/07-merge-pointers-step6.svg){fig-alt="out에 5,8,10,13,14가 채워지고 R의 21이 강조되며 R이 모두 소진되는 상태" width="55%"}

## 7단계

![한쪽이 비면 나머지 suffix를 그대로 붙인다.](../figures/03-sorting/07-merge-pointers-step7.svg){fig-alt="out에 5,8,10,13,14,21이 채워지고 L의 남은 29,37 suffix가 함께 강조된 상태" width="55%"}

## 8단계

![완료: out=[5,8,10,13,14,21,29,37].](../figures/03-sorting/07-merge-pointers-step8.svg){fig-alt="out 배열이 5,8,10,13,14,21,29,37로 완전히 채워진 상태" width="55%"}
:::
...
![병합 비용의 재귀 트리: 각 level 합은 Θ(n), level 수는 Θ(log n)이다.](../figures/03-sorting/07-merge-recursion-tree.svg){fig-alt="루트 n에서 시작해 n/2, n/4로 절반씩 줄어드는 이진 재귀 트리" width="35%"}
...
![$j=1$: 31과 pivot 비교 → $31>15$, 비교만(지나감).](../figures/03-sorting/08-quick-partition-trace-step1.svg){fig-alt="10개 원소 배열에서 마지막 원소 15가 pivot으로, 31이 빗금 표시로 강조된 상태" width="65%"}

## 2단계

![$j=2$: 8과 pivot 비교 → $8\le15$, $i=1$로 확장하며 swap.](../figures/03-sorting/08-quick-partition-trace-step2.svg){fig-alt="8이 왼쪽 경계 index 1 안으로 들어오며 강조된 상태" width="65%"}

## 3단계

![$j=3$: 48과 pivot 비교 → $48>15$, 비교만(swap 없음).](../figures/03-sorting/08-quick-partition-trace-step3.svg){fig-alt="48이 빗금 표시로 강조되고 index 1까지가 경계로 표시된 상태" width="65%"}

## 4단계

![$j=4$: 73과 pivot 비교 → $73>15$, 비교만(swap 없음).](../figures/03-sorting/08-quick-partition-trace-step4.svg){fig-alt="73이 빗금 표시로 강조된 상태" width="65%"}

## 5단계

![$j=5$: 11과 pivot 비교 → $11\le15$, $i=2$로 확장하며 swap.](../figures/03-sorting/08-quick-partition-trace-step5.svg){fig-alt="11이 왼쪽 경계 index 2 안으로 들어오며 강조된 상태" width="65%"}

## 6단계

![$j=6$: 3과 pivot 비교 → $3\le15$, $i=3$으로 확장하며 swap.](../figures/03-sorting/08-quick-partition-trace-step6.svg){fig-alt="3이 왼쪽 경계 index 3 안으로 들어오며 강조된 상태" width="65%"}

## 7단계

![$j=7$: 20과 pivot 비교 → $20>15$, 비교만(swap 없음).](../figures/03-sorting/08-quick-partition-trace-step7.svg){fig-alt="20이 빗금 표시로 강조되고 index 3까지가 경계로 표시된 상태" width="65%"}

## 8단계

![$j=8$: 29와 pivot 비교 → $29>15$, 비교만(swap 없음).](../figures/03-sorting/08-quick-partition-trace-step8.svg){fig-alt="29가 빗금 표시로 강조된 상태" width="65%"}

## 9단계

![$j=9$: 65와 pivot 비교 → $65>15$, 비교만(swap 없음).](../figures/03-sorting/08-quick-partition-trace-step9.svg){fig-alt="65가 빗금 표시로 강조된 상태" width="65%"}

## 10단계

![pivot을 index 4와 swap: partition 완료.](../figures/03-sorting/08-quick-partition-trace-step10.svg){fig-alt="pivot 15가 index 4에 최종 위치하고 왼쪽 partition이 완료된 상태를 중괄호로 표시한 상태" width="65%"}
:::
...
![4와 큰 child 14 비교.](../figures/03-sorting/11-heapify-trace-step1.svg){fig-alt="index 1의 4와 큰 child 14가 비교되는 array/tree 상태" width="55%"}

## 2단계

![swap 후 index 2에서 재개.](../figures/03-sorting/11-heapify-trace-step2.svg){fig-alt="4와 14가 swap되어 index 2에서 재개되는 상태" width="55%"}

## 3단계

![다시 큰 child 8과 swap.](../figures/03-sorting/11-heapify-trace-step3.svg){fig-alt="index 2의 4가 큰 child 8과 swap되어 index 4로 내려가는 상태" width="55%"}

## 4단계

![index 4에서 두 child 2, 1보다 크므로 종료.](../figures/03-sorting/11-heapify-trace-step4.svg){fig-alt="index 4에서 멈추는 상태" width="55%"}
:::
...
![$i=5$: leaf의 parent부터 시작.](../figures/03-sorting/12-build-heap-trace-step1.svg){fig-alt="i=5에서 HEAPIFY를 시작하는 array/tree 상태" width="55%"}

## 2단계

![$i=4$ 완료.](../figures/03-sorting/12-build-heap-trace-step2.svg){fig-alt="i=4의 HEAPIFY가 끝난 상태" width="55%"}

## 3단계

![$i=3$ 완료.](../figures/03-sorting/12-build-heap-trace-step3.svg){fig-alt="i=3의 HEAPIFY가 끝난 상태" width="55%"}

## 4단계

![$i=2$ 완료.](../figures/03-sorting/12-build-heap-trace-step4.svg){fig-alt="i=2의 HEAPIFY가 끝난 상태" width="55%"}

## 5단계

![$i=1$ 완료: max-heap.](../figures/03-sorting/12-build-heap-trace-step5.svg){fig-alt="i=1의 HEAPIFY까지 끝나 16,14,10,8,7,9,3,2,4,1로 완성된 max-heap" width="55%"}
:::
...
![max-heap 준비.](../figures/03-sorting/13-heapsort-trace-step1.svg){fig-alt="16,14,10,8,7,9,3,2,4,1로 이루어진 max-heap 상태" width="55%"}

## 2단계

![16 확정.](../figures/03-sorting/13-heapsort-trace-step2.svg){fig-alt="16이 배열 뒤쪽에 확정되고 active heap이 9개로 줄어든 상태" width="55%"}

## 3단계

![14 확정.](../figures/03-sorting/13-heapsort-trace-step3.svg){fig-alt="14가 확정되고 active heap이 8개로 줄어든 상태" width="55%"}

## 4단계

![10 확정.](../figures/03-sorting/13-heapsort-trace-step4.svg){fig-alt="10이 확정되고 active heap이 7개로 줄어든 상태" width="55%"}

## 5단계

![나머지 동일한 extract+heapify를 반복한 최종 상태.](../figures/03-sorting/13-heapsort-trace-step5.svg){fig-alt="1,2,3,4,7,8,9,10,14,16으로 완전히 정렬된 마지막 상태" width="55%"}
:::
...
![1. initialize.](../figures/03-sorting/14-counting-sort-trace-step1.svg){fig-alt="A=4,1,3,4,3 배열에서 C 배열이 모두 0으로 초기화된 상태" width="60%"}

## 2단계

![2. frequency count.](../figures/03-sorting/14-counting-sort-trace-step2.svg){fig-alt="C 배열이 각 key의 빈도 0,1,0,2,2로 채워진 상태" width="60%"}

## 3단계

![3. cumulative prefix.](../figures/03-sorting/14-counting-sort-trace-step3.svg){fig-alt="C 배열이 누적합 0,1,1,3,5로 바뀐 상태" width="60%"}

## 4단계

![4. stable placement.](../figures/03-sorting/14-counting-sort-trace-step4.svg){fig-alt="B=1,3,3,4,4로 stable하게 배치된 마지막 상태" width="60%"}
:::
...
![ones 결과.](../figures/03-sorting/15-radix-trace-step1.svg){fig-alt="170,90,802,2,24,45,75,66 배열이 ones 자리 기준으로 재배열된 상태" width="60%"}

## 2단계

![tens 결과.](../figures/03-sorting/15-radix-trace-step2.svg){fig-alt="802,2,24,45,66,170,75,90 배열이 tens 자리 기준으로 재배열된 상태" width="60%"}

## 3단계

![hundreds 결과.](../figures/03-sorting/15-radix-trace-step3.svg){fig-alt="2,24,45,66,75,90,170,802로 완전히 정렬된 상태" width="60%"}
:::
...
![Sorting 개념 전체를 한눈에: Properties, Comparison sorting(Simple/Efficient), Key-structure(Counting/Radix)로 정리한다.](../figures/03-sorting/19-concept-map.svg){fig-alt="Sorting을 중심으로 Properties, Comparison sorting, Key-structure/Non-comparison 세 갈래로 나뉘고 각각 세부 항목으로 이어지는 개념 지도" width="55%"}
