...
![Bubble Sort 첫 pass: 인접한 두 원소를 비교해 역전이면 교환한다.](../figures/03-sorting/04-bubble-trace.svg){fig-alt="29,10,14,37,13 배열에서 인접 교환을 거쳐 첫 pass 종료 시 37이 오른쪽 끝에 확정된 상태" width="40%"}
...
![key=13을 shift하며 삽입하는 과정.](../figures/03-sorting/05-insertion-trace.svg){fig-alt="10,14,29,13,37 배열에서 13을 꺼내고 29,14를 오른쪽으로 shift한 뒤 올바른 위치에 삽입해 10,13,14,29,37이 되는 5단계 상태" width="40%"}
...
![Lomuto partition: pivot=15를 기준으로 왼쪽 경계 i를 한 칸씩 넓히며 확정한다.](../figures/03-sorting/08-quick-partition-trace.svg){fig-alt="10개 원소 배열에서 pivot 15가 index 4에 최종 위치하고 왼쪽 partition이 완료된 상태를 중괄호로 표시한 다이어그램" width="65%"}
...
![세 자리 LSD Radix Sort: ones, tens, hundreds 순으로 안정 정렬한다.](../figures/03-sorting/15-radix-trace.svg){fig-alt="170,90,802,2,24,45,75,66 배열이 ones 결과, tens 결과, hundreds 결과 순으로 재배열되어 최종적으로 2,24,45,66,75,90,170,802가 되는 3단계 상태" width="60%"}
