...
지금까지 heap을 정렬 장치로 봤지만, 본질은 **Priority Queue ADT**다 — `Insert`, `ExtractMax`, `Maximum` 연산이 각각 $\Theta(\log n)$, $\Theta(\log n)$, $\Theta(1)$이다. Heapsort는 이 ADT의 한 응용일 뿐이다. Lecture 8의 Prim·Dijkstra와 Lecture 10의 A\*가 모두 이 min-priority queue를 핵심 자료구조로 다시 쓴다.
:::
...
**구현** — Java `Comparator.thenComparing` 체이닝(중복 key 포함)과 C `qsort`의 relational comparator, 그리고 Python `key=` 관용구가 같은 두 성질(extreme value의 오름차순 정렬, 중복 key가 있는 다중 key 정렬의 stability)을 확인한다:
...
::: {.callout-note title="세 언어 결과 비교에 대한 정직한 한계"}
`code/03-sorting/java/FruitSorting.java`와 `code/03-sorting/c/qsort_examples.c`는 `lecture-notes/code/lecture03/`에서 그대로 가져온 원본이며(읽기전용 소스), 서로 **다른 예제 데이터**를 쓴다. 그래서 바이트 단위로 세 출력을 비교하는 것은 의미가 없다. 대신 `scripts/run_examples.py`는 세 구현이 공통으로 보여주는 두 성질 — extreme-value 정수의 오름차순 정렬, 중복 key가 있는 다중 key 정렬의 stability — 을 Python `comparator_demo.py`가 동일하게 만족하는지, 그리고 세 프로그램 모두 자체 assertion을 통과해 성공적으로 종료하는지를 검증한다.
:::
...
**정답**: Selection $[2_b,2_a,4,5]$로 equal record가 역전될 수 있다(unstable). Bubble $[2_a,4,2_b,5]$. Insertion은 5와 4만 shift하고 $2_a$는 움직이지 않는다.
:::
...
**정답**: 1. $2,3,5$. 2. pivot의 최종 index이며 왼쪽은 $\le pivot$, 오른쪽은 $>pivot$이다. 3. parent는 2, children은 8·9다. 4. $[14,8,10,4,7,9,3,2,1,0]$.
:::
...
**정답**: 1. $\Omega(n\log n)$. 2. $\Theta(n)$. 3. Merge·Insertion. 4. $\Theta(n)$. 5. Counting은 작은 finite integer range, LSD Radix는 digit 표현과 stable pass가 필요하다.
:::
...
**정답**: C는 relational comparison `(x>y)-(x<y)`로 바꿔야 한다(overflow 위험). Java는 `.thenComparing(f -> f.name)`을 이어 붙인다.
:::
