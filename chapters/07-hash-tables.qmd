...
**답**: 1. 아니다, order가 필요하다. 2. equality lookup에 적합하다. 3. resize가 없으면 suitable hashing/controlled load 아래 expected $\Theta(1)$, resize 포함 PUT은 amortized expected $\Theta(1)$이며 individual resizing PUT과 worst case는 $\Theta(n)$일 수 있다.
:::
...
**답**: 1. $0\ldots15$. 2. 같은 lookup path를 만들어야 한다. 3. index가 capacity에 의존해 모든 active entry를 재배치해야 한다.
:::
...
**답**: 1. 아니다. 2. 아니다. 3. expected는 randomness에 대한 기댓값, amortized는 연산열 총비용의 평균이다.
:::
...
![$25A\approx15.45085$를 계산한다.](../figures/07-hash-tables/04-multiplication-trace-step1.svg){fig-alt="25A≈15.45085 상자가 강조된 1단계 상태" width="90%"}
...
**답**: 1. 문자 순서를 버리기 때문이다. 2. `hash = hash*B + code(c)`. 3. 5.
:::
...
![$h(29)=29\bmod13=3$.](../figures/07-hash-tables/07-insert-29-collision-trace-step1.svg){fig-alt="slot 3이 16으로 점유된 상태에서 h(29)=3이라는 콜아웃이 붙은 1단계 상태" width="75%"}
...
![$10\bmod7=3$: 빈 bucket에 삽입.](../figures/07-hash-tables/10-chaining-insert-trace-step1.svg){fig-alt="bucket 3에 10 하나만 연결된 1단계 상태" width="70%"}
...
![collision; head insert 24.](../figures/07-hash-tables/10-chaining-insert-trace-step3.svg){fig-alt="bucket 3에 24,17,10 순서로 연결된 최종 상태와 collision; head insert 24라는 콜아웃이 붙은 3단계 상태" width="70%"}
:::
...
![SEARCH 17: compare 24.](../figures/07-hash-tables/11-chaining-search-delete-trace-step1.svg){fig-alt="24가 강조되고 SEARCH 17: compare 24라는 콜아웃이 붙은 1단계 상태" width="70%"}
...
![24 → 10.](../figures/07-hash-tables/11-chaining-search-delete-trace-step4.svg){fig-alt="17이 제거되고 24와 10이 직접 연결된 최종 상태, 24 -> 10이라는 콜아웃이 붙은 4단계 상태" width="70%"}
:::
...
**답**: 1. $12/7$. 2. duplicate/update 여부를 확인하는 search가 필요하다. 3. $24,17,10$.
:::
...
**답**: 1. probe chain에 그 key가 있었다면 EMPTY 이전에 있어야 하기 때문이다. 2. 뒤에 active key가 있을 수 있기 때문이다. 3. resize 또는 명시적 failure.
:::
...
![25,13,16,15,7: 모두 home slot.](../figures/07-hash-tables/14-linear-trace1-collision-step1.svg){fig-alt="25,13,16,15,7이 각자 home slot에 놓인 1단계 상태" width="75%"}
...
![probe $2\to3\to4$.](../figures/07-hash-tables/14-linear-trace1-collision-step3.svg){fig-alt="slot 2,3이 충돌로 표시되고 slot 4에 28이 놓인 3단계 상태" width="75%"}
:::
...
![38 home$=12$.](../figures/07-hash-tables/15-linear-trace2-wraparound-step1.svg){fig-alt="slot 12가 충돌로 표시되고 38 home=12라는 콜아웃이 붙은 1단계 상태" width="75%"}
...
![first EMPTY $=6$.](../figures/07-hash-tables/15-linear-trace2-wraparound-step3.svg){fig-alt="slot 12부터 5까지 충돌로 표시되고 wrap 화살표를 거쳐 slot 6에 38이 놓인 3단계 상태" width="75%"}
:::
...
![cluster length 3.](../figures/07-hash-tables/17-primary-clustering-step1.svg){fig-alt="a,b,c 세 key가 이어진 길이 3 cluster에 brace로 length 3이 표시된 1단계 상태" width="70%"}
...
![cluster length 4.](../figures/07-hash-tables/17-primary-clustering-step3.svg){fig-alt="길이 3이던 cluster가 새 key를 흡수해 길이 4로 늘어난 3단계 상태" width="70%"}
:::
...
**답**: 1. $12,0,1,2,3,4,5,6$에서 6. 2. run에 닿은 key가 끝에 붙어 run을 키우기 때문이다. 3. EMPTY를 만나거나 $m$번 probe 후.
:::
...
![$i=0$: $4+0=4$.](../figures/07-hash-tables/19-quadratic-trace-30-step1.svg){fig-alt="slot 4가 충돌로 표시되고 i=0: 4+0=4라는 콜아웃이 붙은 1단계 상태" width="75%"}
...
![$i=2$: $4+4=8$.](../figures/07-hash-tables/19-quadratic-trace-30-step3.svg){fig-alt="slot 4, 5가 충돌로 표시되고 slot 8에 30이 놓인 3단계 상태" width="75%"}
:::
...
**답**: 1. $4,5,8$. 2. parameter/capacity policy 없이는 아니다. 3. 같은 home key들이 같은 quadratic sequence를 공유하는 현상.
:::
...
![28: $h_1=2,\ h_2=7$.](../figures/07-hash-tables/21-double-hash-probe-trace-step1.svg){fig-alt="slot 2가 충돌로 표시되고 28: h1=2,h2=7이라는 콜아웃이 붙은 1단계 상태" width="75%"}
...
![67: step 2, final 4.](../figures/07-hash-tables/21-double-hash-probe-trace-step4.svg){fig-alt="slot 2,6,9,11에 15,19,28,41이 놓이고 67이 slot 4로 향하는 4단계 상태" width="75%"}
:::
...
**답**: 1. $2,2$, final 4. 2. 모든 slot을 순환하기 위해서다. 3. 가능하다.
:::
...
![DELETE 1.](../figures/07-hash-tables/24-wrong-deletion-trace-step1.svg){fig-alt="slot 1이 강조되고 DELETE 1이라는 콜아웃이 붙은 1단계 상태" width="75%"}
...
![slot 1 EMPTY: false NOT_FOUND.](../figures/07-hash-tables/24-wrong-deletion-trace-step5.svg){fig-alt="slot 1이 EMPTY로 표시되고 slot 1 EMPTY: false NOT_FOUND라는 콜아웃이 붙은 5단계 상태" width="75%"}
:::
...
![DELETE 1 $\Rightarrow$ DELETED.](../figures/07-hash-tables/25-correct-deletion-tombstone-trace-step1.svg){fig-alt="slot 1이 D(tombstone)로 표시되는 1단계 상태" width="75%"}
...
![slot 6: found.](../figures/07-hash-tables/25-correct-deletion-tombstone-trace-step4.svg){fig-alt="slot 1이 D로 표시되고 slot 6에서 38을 찾았다는 콜아웃이 붙은 4단계 상태" width="75%"}
:::
...
**답**: 1. 검색이 slot 1에서 조기 종료되기 때문이다. 2. search는 통과, insert는 후보로 기억한다. 3. active+tombstone 기반 probing load.
:::
...
**답**: 1. 가능하다. 2. EMPTY가 없어 불가능하다. 3. resize 없는 연산은 suitable hashing/controlled load에서 expected $\Theta(1)$, resize 포함 PUT은 amortized expected $\Theta(1)$, individual resize와 worst case는 $\Theta(n)$.
:::
...
![old $m=5$: $h(k)=k\bmod5$. 2: home 2; 7: $2\to3$; 12: $2\to3\to4$. slot 1: earlier deletion의 tombstone.](../figures/07-hash-tables/27-rehash-animation-step1.svg){fig-alt="old table에서 2,7,12가 각자 home과 probe 위치에 놓이고 slot 1이 tombstone인 1단계 상태" width="65%"}
...
**답**: 1. index가 capacity에 의존하기 때문이다. 2. $<2n$. 3. resizing PUT은 $\Theta(n)$, 전체 sequence에서는 PUT당 추가 amortized $O(1)$이고 기본 hashing까지 포함하면 amortized expected $\Theta(1)$.
:::
...
Java의 계약: $a.equals(b)\Rightarrow a.hashCode()=b.hashCode()$이지만, 역은 성립하지 않는다($a.hashCode()=b.hashCode()\not\Rightarrow a.equals(b)$). collision 후 반드시 `equals`로 logical key를 확인해야 하고, key가 table 안에 있는 동안 equality/hash-relevant field를 변경해서는 안 되며, custom key는 두 method를 함께 설계해야 한다.

::: {.callout-warning title="Mutable Key Failure"}
`equals`/`hashCode`가 `id` 필드에만 의존하는 key를 `HashMap`에 넣은 뒤 `key.id`를 바꾸면, entry는 여전히 **원래 id의 bucket**에 남아 있지만 `get(key)`는 **바뀐 id**로 lookup path를 계산한다. Map은 mutated key를 자동으로 재배치할 의무가 없다 — key가 table 안에 있는 동안 `equals`/`hashCode` 관련 field는 immutable이어야 한다.
:::
...
**실행 결과** (실제 컴파일·실행 캡처 — `javac`/`java`, `assert` 없이 관찰값을 직접 출력): `id`를 20으로 바꾼 뒤에는 mutated key 자체로도, `id=10`으로도, `id=20`으로도 **어느 쪽으로도 다시 찾을 수 없다** — entry는 사라지지 않았지만(size는 그대로 1) 도달 불가능해졌다.

```{.default}
{{< include ../figures/07-hash-tables/out-mutable-key-example-java.txt >}}
```

C 구현에서는 chaining은 integer key/value를 entry node가 소유하고 `destroy`가 모든 node와 bucket array를 해제하며, open addressing은 slot array가 value를 직접 보관하고 state enum이 EMPTY/OCCUPIED/DELETED를 구분한다. allocation failure는 false 반환과 원상태 보존으로 처리하고, negative key는 normalized modulo로 처리하며, dangling pointer를 API 밖으로 반환하지 않는다.
