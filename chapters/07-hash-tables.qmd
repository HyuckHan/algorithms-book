...
![$29$를 삽입하려는데 $h(29)=3$이 이미 점유되어 있어 collision이 발생하고, 이후 resolution policy가 실제 위치를 정한다.](../figures/07-hash-tables/07-insert-29-collision-trace-step3.svg){fig-alt="slot 3이 16으로 점유된 상태에서 29를 삽입하려다 충돌이 표시되고 resolution policy가 실제 위치를 정한다는 콜아웃이 붙은 마지막 상태" width="75%"}
...
![25,13,16,15,7이 모두 home slot에 있는 상태에서 28을 삽입하면 home 2가 점유되어 있어 $2\to3\to4$로 probe한다.](../figures/07-hash-tables/14-linear-trace1-collision-step3.svg){fig-alt="slot 2,3이 충돌로 표시되고 slot 4에 28이 놓인 3단계 상태" width="75%"}

![여러 key가 채워진 뒤 38(home 12)을 삽입하면 $12\to0\to1\to\cdots\to5$가 모두 점유되어 있어 wrap-around 끝에 첫 EMPTY인 6에 놓인다.](../figures/07-hash-tables/15-linear-trace2-wraparound-step3.svg){fig-alt="slot 12부터 5까지 충돌로 표시되고 wrap 화살표를 거쳐 slot 6에 38이 놓인 3단계 상태" width="75%"}
...
![home 4가 이미 점유된 상태에서 30을 삽입하면 $i=0,1,2$가 각각 $4,5,8$로 계산되어 slot 8에 놓인다.](../figures/07-hash-tables/19-quadratic-trace-30-step3.svg){fig-alt="slot 4, 5가 충돌로 표시되고 slot 8에 30이 놓인 3단계 상태" width="75%"}
...
![15, 19가 이미 놓인 상태에서 28, 41, 67을 각자의 $h_1,h_2$로 계산한 probe sequence를 따라 배치하는 애니메이션의 마지막 상태.](../figures/07-hash-tables/21-double-hash-probe-trace-step4.svg){fig-alt="slot 2,6,9,11에 15,19,28,41이 놓이고 67이 slot 4로 향하는 4단계 상태" width="75%"}
...
![key 1 삭제를 EMPTY로 처리하면 이후 SEARCH 38이 slot 1의 EMPTY에서 조기 종료해 false NOT_FOUND가 되는 5단계 트레이스의 마지막 상태.](../figures/07-hash-tables/24-wrong-deletion-trace-step5.svg){fig-alt="slot 1이 EMPTY로 표시되고 slot 1 EMPTY: false NOT_FOUND라는 콜아웃이 붙은 5단계 상태" width="75%"}

**올바른 삭제**는 tombstone(DELETED)을 남긴다 — DELETED에서는 계속 probe하므로 SEARCH 38이 slot 1의 D를 통과해 slot 6에서 정상적으로 찾는다.

![key 1 삭제를 DELETED(tombstone)로 처리하면 SEARCH 38이 slot 1의 D를 통과해 slot 6에서 찾는 4단계 트레이스의 마지막 상태.](../figures/07-hash-tables/25-correct-deletion-tombstone-trace-step4.svg){fig-alt="slot 1이 D로 표시되고 slot 6에서 38을 찾았다는 콜아웃이 붙은 4단계 상태" width="75%"}
...
![active 9개/capacity 20의 logical load(0.45)와, tombstone 5개를 더한 probing load(0.70)를 나란히 보여주는 게이지.](../figures/07-hash-tables/26-logical-vs-probing-load-step2.svg){fig-alt="logical load 0.45 게이지와 probing load 0.70 게이지가 나란히 표시된 다이어그램" width="60%"}
...
![old table($m=5$)의 active entry 2,7,12를 new table($m=11$)로 재배치하는 rehash 애니메이션의 마지막 상태 — $12\to1,\ 2\to2,\ 7\to7$.](../figures/07-hash-tables/27-rehash-animation-step4.svg){fig-alt="old table의 2,7,12가 new table의 1,2,7번 슬롯으로 재배치된 최종 상태" width="65%"}
