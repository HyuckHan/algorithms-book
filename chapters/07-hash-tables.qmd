...
![같은 pattern을 보존해 특정 bucket에 몰리는 나쁜 분산과, table 전체로 고르게 퍼진 좋은 분산을 나란히 비교한다.](../figures/07-hash-tables/03-distribution-comparison.svg){fig-alt="위쪽은 몇몇 막대가 유독 높은 나쁜 분산, 아래쪽은 막대 높이가 고른 좋은 분산을 보여주는 두 막대그래프" width="70%"}
...
![Multiplication trace $k=25,\ m=13$: $25A\approx15.451$ → frac $=0.451$ → $13\times0.451\approx5.861$ → $\lfloor5.861\rfloor=5$.](../figures/07-hash-tables/04-multiplication-trace-step4.svg){fig-alt="25A, frac, 13*frac, floor 네 단계를 거쳐 최종 index 5에 도달하는 계산 트레이스의 마지막 상태" width="90%"}
...
![$m=13$ table에 $13,15,16,7,25$가 각자의 home slot에 놓인 상태.](../figures/07-hash-tables/06-original-integer-table.svg){fig-alt="13칸짜리 hash table에서 0,2,3,7,12번 슬롯에 각각 13,15,16,7,25가 채워지고 나머지는 비어있는 다이어그램" width="90%"}

이제 $29$를 삽입하면 $h(29)=29\bmod13=3$인데 slot 3은 이미 $16$이 차지하고 있다 — collision이다.

![$29$를 삽입하려는데 $h(29)=3$이 이미 점유되어 있어 collision이 발생하고, 이후 resolution policy가 실제 위치를 정한다.](../figures/07-hash-tables/07-insert-29-collision-trace-step3.svg){fig-alt="slot 3이 16으로 점유된 상태에서 29를 삽입하려다 충돌이 표시되고 resolution policy가 실제 위치를 정한다는 콜아웃이 붙은 마지막 상태" width="90%"}

$$h(k_1)=h(k_2),\quad k_1\ne k_2.$$

**Pigeonhole principle**: $|U|>m$이면 $h(k_1)=h(k_2)$인 서로 다른 $k_1,k_2$가 반드시 존재한다.

![key 4개가 bucket 3개로 매핑되어야 하므로 적어도 두 key가 같은 bucket을 공유하는 pigeonhole 다이어그램.](../figures/07-hash-tables/08-pigeonhole-principle.svg){fig-alt="key a,b,c,d가 bucket 0,1,2로 향하는 화살표 중 두 key가 같은 bucket을 가리키는 다이어그램" width="60%"}
...
![10 삽입(빈 bucket) → 17 삽입(collision, head insert) → 24 삽입(collision, head insert)까지 3단계.](../figures/07-hash-tables/10-chaining-insert-trace-step3.svg){fig-alt="bucket 3에 24,17,10 순서로 연결된 최종 상태와 collision; head insert 24라는 콜아웃이 붙은 3단계 상태" width="70%"}

![SEARCH 17(24와 비교 후 17에서 발견) → DELETE 17(unlink) 이후 24와 10이 직접 연결되는 4단계.](../figures/07-hash-tables/11-chaining-search-delete-trace-step4.svg){fig-alt="17이 제거되고 24와 10이 직접 연결된 최종 상태, 24 -> 10이라는 콜아웃이 붙은 4단계 상태" width="70%"}
...
![tombstone(D)이 있는 table: 새 key를 첫 D에 바로 쓰지 않고 EMPTY까지 duplicate 여부를 계속 확인한 뒤 재사용한다.](../figures/07-hash-tables/13-tombstone-reuse-example.svg){fig-alt="D, 42, 29 등이 섞인 13칸 hash table 한 줄" width="90%"}
...
![25,13,16,15,7이 모두 home slot에 있는 상태에서 28을 삽입하면 home 2가 점유되어 있어 $2\to3\to4$로 probe한다.](../figures/07-hash-tables/14-linear-trace1-collision-step3.svg){fig-alt="slot 2,3이 충돌로 표시되고 slot 4에 28이 놓인 3단계 상태" width="90%"}

![여러 key가 채워진 뒤 38(home 12)을 삽입하면 $12\to0\to1\to\cdots\to5$가 모두 점유되어 있어 wrap-around 끝에 첫 EMPTY인 6에 놓인다.](../figures/07-hash-tables/15-linear-trace2-wraparound-step3.svg){fig-alt="slot 12부터 5까지 충돌로 표시되고 wrap 화살표를 거쳐 slot 6에 38이 놓인 3단계 상태" width="90%"}
...
![10-key 삽입이 끝난 최종 table: `13,1,15,16,28,31,38,7,20,E,E,E,25`.](../figures/07-hash-tables/16-linear-final-table.svg){fig-alt="13칸 hash table에 13,1,15,16,28,31,38,7,20이 채워지고 세 칸은 비어있으며 마지막 칸에 25가 있는 다이어그램" width="90%"}

Primary clustering은 **자기 강화**된다 — run 앞이나 내부로 hash된 key가 끝에 붙고, 더 긴 run은 더 넓은 home range의 key를 흡수한다.

![길이 3인 cluster에 home이 cluster 내부인 새 key가 들어와 run 끝에 붙으면서 cluster가 길이 4로 자라는 과정.](../figures/07-hash-tables/17-primary-clustering-step3.svg){fig-alt="길이 3이던 cluster가 새 key를 흡수해 길이 4로 늘어난 3단계 상태" width="70%"}
...
![home 4가 이미 점유된 상태에서 30을 삽입하면 $i=0,1,2$가 각각 $4,5,8$로 계산되어 slot 8에 놓인다.](../figures/07-hash-tables/19-quadratic-trace-30-step3.svg){fig-alt="slot 4, 5가 충돌로 표시되고 slot 8에 30이 놓인 3단계 상태" width="90%"}
...
![home이 모두 2인 15, 28, 41이 같은 quadratic probe sequence를 공유하는 다이어그램.](../figures/07-hash-tables/20-secondary-clustering.svg){fig-alt="home 2를 공유하는 세 key가 모두 같은 probe sequence 상자로 화살표를 뻗는 다이어그램" width="60%"}
...
![15, 19가 이미 놓인 상태에서 28, 41, 67을 각자의 $h_1,h_2$로 계산한 probe sequence를 따라 배치하는 애니메이션의 마지막 상태.](../figures/07-hash-tables/21-double-hash-probe-trace-step4.svg){fig-alt="slot 2,6,9,11에 15,19,28,41이 놓이고 67이 slot 4로 향하는 4단계 상태" width="90%"}
...
![step 4와 capacity 12가 서로소가 아니어서 3개 slot만 순환하고 나머지는 도달하지 못하는 다이어그램.](../figures/07-hash-tables/22-double-hash-gcd-failure.svg){fig-alt="12칸 중 세 칸만 순환 대상으로 표시된 다이어그램" width="90%"}
...
![삭제 전 probe cluster: `13,1,15,16,28,31,38,7,20,E,E,E,25`.](../figures/07-hash-tables/23-probe-cluster-before-delete.svg){fig-alt="13칸 hash table에 13,1,15,16,28,31,38,7,20이 채워지고 세 칸은 비어있으며 마지막 칸에 25가 있는 다이어그램" width="90%"}
...
![key 1 삭제를 EMPTY로 처리하면 이후 SEARCH 38이 slot 1의 EMPTY에서 조기 종료해 false NOT_FOUND가 되는 5단계 트레이스의 마지막 상태.](../figures/07-hash-tables/24-wrong-deletion-trace-step5.svg){fig-alt="slot 1이 EMPTY로 표시되고 slot 1 EMPTY: false NOT_FOUND라는 콜아웃이 붙은 5단계 상태" width="90%"}

**올바른 삭제**는 tombstone(DELETED)을 남긴다 — DELETED에서는 계속 probe하므로 SEARCH 38이 slot 1의 D를 통과해 slot 6에서 정상적으로 찾는다.

![key 1 삭제를 DELETED(tombstone)로 처리하면 SEARCH 38이 slot 1의 D를 통과해 slot 6에서 찾는 4단계 트레이스의 마지막 상태.](../figures/07-hash-tables/25-correct-deletion-tombstone-trace-step4.svg){fig-alt="slot 1이 D로 표시되고 slot 6에서 38을 찾았다는 콜아웃이 붙은 4단계 상태" width="90%"}
...
![active 9개/capacity 20의 logical load(0.45)와, tombstone 5개를 더한 probing load(0.70)를 나란히 보여주는 게이지.](../figures/07-hash-tables/26-logical-vs-probing-load-step2.svg){fig-alt="logical load 0.45 게이지와 probing load 0.70 게이지가 나란히 표시된 다이어그램" width="70%"}
...
![old table($m=5$)의 active entry 2,7,12를 new table($m=11$)로 재배치하는 rehash 애니메이션의 마지막 상태 — $12\to1,\ 2\to2,\ 7\to7$.](../figures/07-hash-tables/27-rehash-animation-step4.svg){fig-alt="old table의 2,7,12가 new table의 1,2,7번 슬롯으로 재배치된 최종 상태" width="90%"}
