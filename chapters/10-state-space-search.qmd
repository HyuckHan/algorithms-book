...
![State-space expansion 4단계: root만 생성 → 필요한 두 child 생성 → DFS는 L subtree부터 → prune node의 후손은 생성하지 않음.](../figures/10-state-space-search/02-state-space-expansion-step4.svg){fig-alt="root에서 두 child, 다시 각 child에서 두 grandchild로 확장되며 마지막 하나는 prune되어 후손이 생성되지 않는 4단계 트레이스의 마지막 상태" width="50%"}
