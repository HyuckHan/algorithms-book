...
![1을 선택 — [1]. 원소가 하나뿐이라 공차는 아직 정해지지 않았다.](../figures/10-state-space-search/11-arithmetic-progression-trace-step1.svg){fig-alt="state box에 choose 1: [1]이 표시되고 note에 difference unset이라고 쓰인 상태" width="30%"}

## 2단계

![3을 선택 — [1,3]. 여기서 공차 d=2가 고정된다.](../figures/10-state-space-search/11-arithmetic-progression-trace-step2.svg){fig-alt="state box가 choose 3: [1,3],d=2로 갱신되고 note에 difference fixed라고 표시된 상태" width="30%"}

## 3단계

![다음으로 4를 시도하면 인접 차이가 2와 1로 갈려 등차 조건 위반 — 이 가지를 prune.](../figures/10-state-space-search/11-arithmetic-progression-trace-step3.svg){fig-alt="state box에 try 4: differences 2,1⇒P가 표시되고 note에 constraint prune이라고 쓰인 상태" width="30%"}

## 4단계

![대신 5와 7을 이어 선택 — [1,3,5,7], 모든 인접 차이가 2로 유지된다.](../figures/10-state-space-search/11-arithmetic-progression-trace-step4.svg){fig-alt="state box가 choose 5,7: [1,3,5,7]로 바뀌고 note에 same difference 2라고 표시된 상태" width="30%"}

## 5단계

![길이 4의 등차수열 완성, incumbent를 INC←4로 갱신.](../figures/10-state-space-search/11-arithmetic-progression-trace-step5.svg){fig-alt="state box에 complete feasible: INC←4가 표시되고 note에 INC=4 badge가 나타난 상태" width="30%"}
:::
...
![A* OPEN/CLOSED trace: S를 추출해 CLOSED로 → neighbor들이 OPEN에 들어감 → 현재 최소 f를 계속 추출 → wall이 frontier를 아래로 유도 → goal 발견(g=10) → non-stale minimum으로 G를 추출해 종료.](../figures/10-state-space-search/17-astar-open-closed-trace-step6.svg){fig-alt="시작 지점부터 장애물을 피해 격자를 확장하며 CLOSED로 확정된 칸들이 늘어나고 최종적으로 목표 지점을 g=10에 추출하는 6단계 트레이스의 마지막 상태" width="65%"}
