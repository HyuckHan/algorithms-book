...
![State-space expansion 4단계: root만 생성 → 필요한 두 child 생성 → DFS는 L subtree부터 → prune node의 후손은 생성하지 않음.](../figures/10-state-space-search/02-state-space-expansion-step4.svg){fig-alt="root에서 두 child, 다시 각 child에서 두 grandchild로 확장되며 마지막 하나는 prune되어 후손이 생성되지 않는 4단계 트레이스의 마지막 상태" width="70%"}
...
![Permutation state tree: state=선택한 prefix → decision=unused value → used가 duplicate branch를 차단 → leaf마다 길이 k output.](../figures/10-state-space-search/04-permutation-state-tree-step4.svg){fig-alt="빈 리스트에서 시작해 unused value를 선택하며 확장되고 leaf마다 길이 k인 output을 만드는 4단계 트레이스의 마지막 상태" width="70%"}
...
![Apply와 Undo의 대칭: decision을 state에 반영 → promising이면 재귀 → return 후 Undo하면 sibling은 동일한 parent state에서 시작한다.](../figures/10-state-space-search/05-apply-undo-symmetry-step3.svg){fig-alt="state S에서 Apply로 자식 상태로 갔다가 recurse한 뒤 Undo로 다시 정확히 원래 state S로 돌아오는 3단계 트레이스의 마지막 상태" width="65%"}
...
![4-Queens: row 0 candidate col 1 → position=[1] → position=[1,3] promising → row 2 col 2 diagonal conflict, backtrack → row 2 col 0 promising → 완성 [1,3,0,2].](../figures/10-state-space-search/06-four-queens-board-trace-step6.svg){fig-alt="4x4 체스판에 퀸을 하나씩 놓으며 충돌 시 backtrack하고 최종적으로 [1,3,0,2] 해를 완성하는 6단계 트레이스의 마지막 상태" width="75%"}

![4-Queens partial state tree: [1,3,0] conflict로 dead, [1,3,0,2]와 [2,0,3,1] 두 solution.](../figures/10-state-space-search/07-four-queens-partial-tree.svg){fig-alt="빈 상태에서 [1], [1,3]으로 확장되고 [1,3,0]은 conflict로 dead, [1,3,0,2]가 valid인 부분 상태 트리, 또 다른 가지 [2,0,3,1]도 valid로 표시된 다이어그램" width="60%"}
...
![Subset Sum trace: i=0,s=0,r=18 → include 3 → include 4,5는 s=12>9로 prune → exclude 후 {3,6} 확정 → 다른 branch {4,5}도 확정.](../figures/10-state-space-search/09-subset-sum-trace-step5.svg){fig-alt="i=0에서 시작해 3을 포함한 뒤 4,5를 포함하면 overshoot으로 prune되고 결국 두 solution을 모두 찾는 5단계 트레이스의 마지막 상태" width="70%"}
...
![작은 4-vertex graph coloring trace: A:1 → B:2 → C:1은 A와 conflict로 prune → C:3, D:2로 valid 3-coloring 완성.](../figures/10-state-space-search/10-graph-coloring-trace-step4.svg){fig-alt="A,B,C,D 네 vertex 그래프에서 A에 색 1, B에 색 2를 배정한 뒤 C에 색 1을 시도했다가 conflict로 prune하고 결국 C:3, D:2로 3-coloring을 완성하는 4단계 트레이스의 마지막 상태" width="70%"}
...
![등차수열 trace: choose 1 → choose 3(d=2) → try 4는 차이 2,1로 prune → choose 5,7으로 [1,3,5,7] 완성 → INC=4.](../figures/10-state-space-search/11-arithmetic-progression-trace-step5.svg){fig-alt="1을 선택하고 3을 선택해 차이 2를 고정한 뒤 4를 시도하다 차이가 달라 prune하고 5,7을 이어 선택해 길이 4의 등차수열을 완성하는 5단계 트레이스의 마지막 상태" width="75%"}
...
![5×7 grid: 장애물(회색)을 피해 S에서 G로 가는 경로를 찾는다.](../figures/10-state-space-search/16-astar-grid-heuristic.svg){fig-alt="5행 7열 격자에서 여섯 개의 장애물 칸이 회색으로 표시되고 왼쪽 위에 S, 오른쪽 아래에 G가 있는 다이어그램" width="55%"}
...
![Path reconstruction: goal에서 parent chain을 거꾸로 따라가 S까지의 전체 경로를 복원한다.](../figures/10-state-space-search/18-astar-path-reconstruction-step3.svg){fig-alt="목표 지점에서 시작 지점까지 parent 체인을 따라 복원된 전체 경로가 화살표로 표시된 3단계 트레이스의 마지막 상태" width="55%"}
