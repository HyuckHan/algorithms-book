...
![Path reconstruction: goal에서 parent chain을 거꾸로 따라가 S까지의 전체 경로를 복원한다.](../figures/10-state-space-search/18-astar-path-reconstruction-step3.svg){fig-alt="목표 지점에서 시작 지점까지 parent 체인을 따라 복원된 전체 경로가 화살표로 표시된 3단계 트레이스의 마지막 상태" width="35%"}
