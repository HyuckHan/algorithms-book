...
![root를 확장한다 — $UB=115$, PQ에 root 하나.](../figures/10-state-space-search/13-knapsack-best-first-trace-step1.svg){fig-alt="state box에 root (w0,p0,UB115)가, note에 PQ view root:115가 표시된 상태" width="26%"}

## 2단계

![A를 포함한다 — $(w2,p40)$, UB는 여전히 115.](../figures/10-state-space-search/13-knapsack-best-first-trace-step2.svg){fig-alt="state box에 A를 포함한 (w2,p40,UB115)가, note에 PQ view A:115가 표시된 상태" width="30%"}

## 3단계

![A를 제외한 가지도 만든다 — $(w0,p0)$, $UB=82$.](../figures/10-state-space-search/13-knapsack-best-first-trace-step3.svg){fig-alt="state box에 A를 제외한 (w0,p0,UB82)가, note에 PQ view A:115와 ¬A:82가 표시된 상태" width="30%"}

## 4단계

![B 포함·제외 두 자식이 생겨 PQ는 $AB{:}115$, $A\lnot B{:}98$, $\lnot A{:}82$이 된다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step4.svg){fig-alt="state box에 B 포함·제외 두 자식 생성이, note에 PQ view AB:115, A¬B:98, ¬A:82가 표시된 상태" width="30%"}

## 5단계

![UB가 가장 큰 AB를 꺼낸다 — C를 넣으면 무게 17로 용량 16을 넘어 폐기되고, $AB\lnot C$가 $UB=80$으로 남는다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step5.svg){fig-alt="state box에 AB pop 후 C 포함 시 무게 17로 폐기, AB¬C는 UB80으로 잔류한다는 내용이, note에 PQ view A¬B:98, ¬A:82, AB¬C:80이 표시된 상태" width="30%"}

## 6단계

![다음으로 $A\lnot B$를 꺼내 C를 포함한다 — $w12,p90$으로 incumbent 확정.](../figures/10-state-space-search/13-knapsack-best-first-trace-step6.svg){fig-alt="state box에 exclude-B를 확장해 C를 포함한 (w12,p90)이, note에 incumbent 90: selected {A,C}가 표시된 상태" width="30%"}

## 7단계

![$UB$가 90 이하인 $\lnot A(82)$와 $AB\lnot C(80)$를 모두 자른다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step7.svg){fig-alt="state box에 incumbent 90 이하인 ¬A(82), AB¬C(80) prune이, note에 objective-bound prune이 표시된 상태" width="30%"}
