...
![State-space tree의 구조: root(결정 0개)에서 A·B를 고르고 다시 C·D를 골라 leaf 넷에 도달한다. {A,C}는 valid, {A,D}는 dead, {B,D}는 bounded다.](../figures/10-state-space-search/01-state-space-tree-structure.svg){fig-alt="빈 상태를 root로 A, B 두 child로 갈라지고 다시 각각 C, D로 확장되어 leaf 넷에 도달하는 트리. {A,C}는 valid, {A,D}는 dead, {B,D}는 bounded로 표시된다" width="65%"}
...
::: {.callout-note collapse="true" title="핵심 정리: complete·feasible·optimal"}
...
![L을 확장해 LL·LR이 생성된다. LR은 dead — 어떤 후손도 feasible일 수 없어 더 내려가지 않는다.](../figures/10-state-space-search/02-state-space-expansion-step3.svg){fig-alt="L 노드 아래에 valid 상태 LL과 dead 상태 LR 두 자식이 추가된 상태" width="46%"}

## 4단계

![R을 확장해 RL·RR이 생성된다. RR은 bound에 걸려 후손이 아예 만들어지지 않는다. LR(dead)과 RR(bound)이 가지치기의 두 유형이다.](../figures/10-state-space-search/02-state-space-expansion-step4.svg){fig-alt="R 노드 아래에 valid 상태 RL과 bound 상태 RR 두 자식이 추가되고, 왼쪽의 dead 상태 LR도 함께 표시된 상태" width="53%"}
:::
...
![같은 tree, 다른 방문 순서: DFS는 R,A,C,D,B,E,F, BFS는 R,A,B,C,D,E,F 순으로 방문하지만 best-first는 tree 구조가 같고 frontier 정책만 다르다.](../figures/10-state-space-search/03-dfs-bfs-visit-order-step3.svg){fig-alt="같은 7-node 트리에서 DFS 순서와 BFS 순서를 비교하고 best-first는 frontier 정책만 다르다는 콜아웃이 붙은 그림" width="60%"}
...
1. 아니다. 
2. 한 level의 많은 live node를 보관하기 때문이다. 
3. 아니다, 최악에는 exponential이다.
:::
...
`ChoosePermutation`은 $\{0,\ldots,n-1\}$에서 중복 없이 $k$개를 고르는 0-based 알고리즘이다 — 예를 들어 `pick(5,...,4)`는 $0,\ldots,4$에서 중복 없이 4개를 고르는 ${}_5P_4$ 생성을 의미한다.

{{< include ../figures/10-state-space-search/pseudocode-choose-permutation.qmd >}}

![P(5,4) state tree의 일부: state는 선택한 prefix, decision은 아직 쓰지 않은 value다. [0,1]에서 0과 1은 이미 used라 막히고 2·3·4로만 뻗는다. 점선과 ⋯은 생략된 subtree이며, 길이 4에 도달한 [0,1,2,3]이 순열 하나에 대응한다.](../figures/10-state-space-search/04-permutation-state-tree-step4.svg){fig-alt="root에서 [0], [0,1]로 이어지는 트리. [0,1]의 자식인 [0,1,0]과 [0,1,1]에는 used 라벨이 붙어 막혀 있고, [0,1,2]가 [0,1,2,3]까지 뻗어 길이 4 output을 완성한다. 나머지 후보와 하위 가지는 점선과 ⋯으로 생략되어 있다" width="55%"}
...
**구현**: ${}_5P_4$와 $\binom54$를 실제로 세어 검증한다.
...
1. `used[v]`가 참 iff $v$가 prefix에 정확히 한 번 존재한다. 
2. 같은 집합의 다른 순서를 생성하지 않기 위해서다.
:::
...
![Apply와 Undo의 대칭: decision을 state에 반영 → promising이면 재귀 → return 후 Undo하면 sibling은 동일한 parent state에서 시작한다.](../figures/10-state-space-search/05-apply-undo-symmetry-step3.svg){fig-alt="state S에서 Apply로 자식 상태로 갔다가 recurse한 뒤 Undo로 다시 정확히 원래 state S로 돌아오는 그림" width="60%"}
...
![4-Queens의 해 [1,3,0,2]: 네 퀸이 서로 열도 대각선도 공유하지 않는 배치.](../figures/10-state-space-search/06-four-queens-board-trace-step5.svg){fig-alt="4x4 체스판에 네 퀸이 row 0 col 1, row 1 col 3, row 2 col 0, row 3 col 2에 놓여 서로 충돌하지 않는 완성된 배치" width="45%"}

![4-Queens partial state tree: col 0 서브트리는 두 candidate가 모두 소진돼 통째로 실패하고, col 1 쪽은 [1,3,0]을 거쳐 [1,3,0,2]를 찾으며, col 2 쪽에 두 번째 해 [2,0,3,1]이 있다.](../figures/10-state-space-search/07-four-queens-partial-tree.svg){fig-alt="빈 상태에서 col 0, col 1, col 2, col 3 네 자식으로 확장되는 부분 상태 트리. col 0 서브트리는 [0,2]와 [0,3,1] 두 후보가 모두 소진돼 dead로 표시되고, col 1 서브트리는 [1,3]과 [1,3,0]을 거쳐 [1,3,0,2]가 valid solution으로 표시된다. col 2 서브트리는 생략 표기 뒤에 두 번째 valid solution [2,0,3,1]이 있고, col 3 서브트리는 생략 표기만 있다." width="65%"}
...
::: {.callout-warning collapse="true" title="주의: undo에서 복구할 것"}
...
![Include/Exclude tree: [3,4,5,6], W=9 — {3,6}과 {4,5} 두 solution. s>W인 가지는 overshoot로, s+r<W인 가지는 목표에 도달할 수 없어 각각 다른 이유로 잘린다.](../figures/10-state-space-search/08-subset-sum-include-exclude-tree.svg){fig-alt="i0,s0,r18에서 시작하는 12노드 include/exclude 트리. {3,6}과 {4,5} 두 solution은 초록색, s>W로 overshoot한 {3,4,5}와 s+r<W로 unreachable한 빈 집합 두 dead 노드는 빨간색으로 표시되고, 나머지 미탐색 가지는 점선과 생략 부호로 표시된다" width="100%"}
...
1. empty subset을 한 해로 즉시 보고한다. 
2. 남은 양수를 모두 포함해도 target에 못 미치기 때문이다.
:::
...
![정점 순서 A, B, C, D를 정하고 아직 아무 색도 배정하지 않는다.](../figures/10-state-space-search/10-graph-coloring-trace-step1.svg){fig-alt="A, B, C, D 네 정점과 다섯 간선(A-B, B-C, C-D, D-A, A-C)만 있고 아직 어떤 정점에도 색이 없는 그래프" width="55%"}

## 2단계

![A는 첫 정점이라 이웃이 아직 색이 없으므로 색 1이 바로 promising하다.](../figures/10-state-space-search/10-graph-coloring-trace-step2.svg){fig-alt="A에 색 1이 배정되고 나머지 B, C, D는 아직 무색인 그래프" width="55%"}

## 3단계

![B는 색 1을 시도하면 이웃 A와 충돌해 기각하고, 색 2로 확정한다.](../figures/10-state-space-search/10-graph-coloring-trace-step3.svg){fig-alt="A:1 옆에 B가 ×1 표시와 함께 색 2로 확정되고, C, D는 아직 무색인 그래프" width="55%"}

## 4단계

![C는 A·B·D 전부와 인접해 색 1은 A와, 색 2는 B와 충돌하므로 색 3으로 확정한다.](../figures/10-state-space-search/10-graph-coloring-trace-step4.svg){fig-alt="C 옆에 ×1과 ×2 두 기각 표시가 붙고 색 3으로 확정되며, D는 아직 무색인 그래프" width="55%"}

## 5단계

![D는 이웃이 A, C뿐이라(B와는 인접하지 않는다) 색 1만 A와 충돌하고 색 2는 promising해 valid 3-coloring이 완성된다.](../figures/10-state-space-search/10-graph-coloring-trace-step5.svg){fig-alt="D 옆에 ×1 표시가 붙고 색 2로 확정되어 네 정점 모두 색이 채워진 valid 3-coloring 그래프" width="55%"}
:::
...
2. upper bound $2+2=4$라 개선 불가, objective-bound pruning.
:::
...
![root를 확장해 UB 16과 UB 12 두 child를 얻는다.](../figures/10-state-space-search/12-bound-based-pruning-step1.svg){fig-alt="UB 20인 root 아래에 UB 16과 UB 12 두 child가 연결된 트리" width="38%"}

## 2단계

![UB 16 쪽을 내려가 value 14인 feasible solution을 찾는다.](../figures/10-state-space-search/12-bound-based-pruning-step2.svg){fig-alt="UB 16 아래에 value 14 노드가 추가되어 feasible solution이 표시된 트리" width="38%"}

## 3단계

![incumbent를 14로 갱신한다 — 이보다 못한 후보는 이제 쓸모없다.](../figures/10-state-space-search/12-bound-based-pruning-step3.svg){fig-alt="value 14 아래에 INC = 14 배지가 붙어 incumbent가 표시된 트리" width="38%"}

## 4단계

![UB 12인 child는 14를 넘을 수 없으므로 잘라낸다.](../figures/10-state-space-search/12-bound-based-pruning-step4.svg){fig-alt="UB 12 위치에 P 표시와 UB 12 ≤ 14 주석이 붙어 해당 subtree가 prune된 트리" width="50%"}
:::
...
![root를 확장한다 — $UB=115$, PQ에 root 하나.](../figures/10-state-space-search/13-knapsack-best-first-trace-step1.svg){fig-alt="state box에 root (w0,p0,UB115)가, note에 PQ view root:115가 표시된 상태" width="24%"}

## 2단계

![A를 포함한다 — $(w2,p40)$, UB는 여전히 115.](../figures/10-state-space-search/13-knapsack-best-first-trace-step2.svg){fig-alt="state box에 A를 포함한 (w2,p40,UB115)가, note에 PQ view A:115가 표시된 상태" width="28%"}

## 3단계

![A를 제외한 가지도 만든다 — $(w0,p0)$, $UB=82$.](../figures/10-state-space-search/13-knapsack-best-first-trace-step3.svg){fig-alt="state box에 A를 제외한 (w0,p0,UB82)가, note에 PQ view A:115와 ¬A:82가 표시된 상태" width="26%"}

## 4단계

![B 포함·제외 두 자식이 생겨 PQ는 $AB{:}115$, $A\lnot B{:}98$, $\lnot A{:}82$이 된다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step4.svg){fig-alt="state box에 B 포함·제외 두 자식 생성이, note에 PQ view AB:115, A¬B:98, ¬A:82가 표시된 상태" width="35%"}

## 5단계

![UB가 가장 큰 AB를 꺼낸다 — C를 넣으면 무게 17로 용량 16을 넘어 폐기되고, $AB\lnot C$가 $UB=80$으로 남는다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step5.svg){fig-alt="state box에 AB pop 후 C 포함 시 무게 17로 폐기, AB¬C는 UB80으로 잔류한다는 내용이, note에 PQ view A¬B:98, ¬A:82, AB¬C:80이 표시된 상태" width="60%"}

## 6단계

![다음으로 $A\lnot B$를 꺼내 C를 포함한다 — $w12,p90$으로 incumbent 확정.](../figures/10-state-space-search/13-knapsack-best-first-trace-step6.svg){fig-alt="state box에 exclude-B를 확장해 C를 포함한 (w12,p90)이, note에 incumbent 90: selected {A,C}가 표시된 상태" width="39%"}

## 7단계

![$UB$가 90 이하인 $\lnot A(82)$와 $AB\lnot C(80)$를 모두 자른다.](../figures/10-state-space-search/13-knapsack-best-first-trace-step7.svg){fig-alt="state box에 incumbent 90 이하인 ¬A(82), AB¬C(80) prune이, note에 objective-bound prune이 표시된 상태" width="47%"}

## 8단계

![PQ가 비었다 — optimum은 weight 12, profit 90.](../figures/10-state-space-search/13-knapsack-best-first-trace-step8.svg){fig-alt="state box에 PQ empty: optimum 90이, note에 optimal weight 12, profit 90이 표시된 화면" width="30%"}
:::

![핵심 state tree(node label: weight/profit/UB): root 0/0/115에서 A 포함 2/40/115와 A 제외 0/0/82로 갈라진다. A 포함 쪽은 다시 B 포함 7/70/115와 B 제외 2/40/98로 나뉜다. B 포함 쪽은 C를 포함하면 weight 17로 용량을 초과해 폐기(w>W)되고, C를 제외하면 12/80/80으로 남지만 UB 80이 incumbent 90을 넘지 못해 prune된다. B 제외 쪽은 C를 포함해 12/90/90에 도달한다. A 제외 쪽은 UB 82가 incumbent 90을 넘지 못해 prune(P)된다.](../figures/10-state-space-search/14-knapsack-state-tree.svg){fig-alt="root 0/0/115에서 A 포함 2/40/115와 A 제외 0/0/82로 갈라지고, A 포함 쪽이 다시 7/70/115와 2/40/98 두 형제로 나뉜다. 7/70/115 아래에는 용량 초과로 폐기된 17/-와 UB80으로 prune된 12/80/80 두 자식이 있고, 2/40/98 아래에는 완성된 해 12/90/90이 있다. A 제외 쪽 아래에는 prune을 뜻하는 P 노드가 있다." width="65%"}

$$currentWeight=\sum_{i\in selected}w_i,$$
...
1. FIFO 또는 넓은 best-first frontier. 
2. upper bound가 큰 순서.
:::
...
![탐색이 끝나 goal G가 확정된 상태.](../figures/10-state-space-search/18-astar-path-reconstruction-step1.svg){fig-alt="격자에서 목표 칸 G만 확정 표시되고 나머지 칸은 비어 있는 상태" width="60%"}

## 2단계

![parent 포인터를 거꾸로 따라가며 화살표로 경로를 역추적한다.](../figures/10-state-space-search/18-astar-path-reconstruction-step2.svg){fig-alt="목표에서 시작까지 격자를 따라 화살표들이 이어지고 시작 칸 S가 표시된 상태" width="65%"}
...
1. 아니다, 직접 backtracking/branch-and-bound 재귀로 구현했다. 
2. Java는 실제 solution(순열, queen 위치, subset, color, 선택 item, path)을, C는 개수·metrics·최적값을 반환한다 — 이 장의 데모는 두 언어가 공통으로 낼 수 있는 값(개수, 최적 profit/weight, cost, graph-coloring의 실제 color 배열)만 비교한다.
:::
