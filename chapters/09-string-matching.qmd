...
![Naive alignment: $P=$`ABA`가 $s=1$에서 $T[1..3]$과 문자별로 일치해 match at 1로 확정된다.](../figures/09-string-matching/01-naive-alignment-trace-step4.svg){fig-alt="C,A,B,A,B,A,C 텍스트 위에서 s=1에 A,B,A 패턴을 맞춰 세 문자가 모두 일치해 match로 확정된 상태" width="45%"}

$$\#alignments=n-m+1,\qquad\#comparisons\le(n-m+1)m$$

alignment 하나가 곧 comparison 하나는 아니다 — mismatch 위치에 따라 같은 alignment의 비용이 달라진다.

`T=AAAAAAAAAAB`, `P=AAAAAB`는 repeated-prefix 최악 예다.

::: {.panel-tabset}
## 1단계

![$s=0$: `P`의 `AAAAA`는 일치하지만 $P[5]=$`B`가 $T[5]=$`A`와 mismatch — 한 shift에 5 match + 1 mismatch.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step1.svg){fig-alt="AAAAAAAAAAB에서 s=0으로 맞춘 AAAAAB가 5문자까지 일치하다가 6번째에서 mismatch가 나는 상태" width="30%"}

## 2단계

![$s=1$: 앞서 본 A들을 다시 비교한다 — $s=1$부터 $s=4$까지 매 shift가 5개의 A를 재확인하고 B에서 mismatch.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step2.svg){fig-alt="s=1로 이동해 같은 긴 prefix를 다시 비교하는 상태" width="30%"}

## 3단계

![$s=5$: 5개의 A에 이어 마지막 B까지 일치 — occurrence 5.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step3.svg){fig-alt="AAAAA 반복 뒤 mismatch가 나는 최악 사례에서 s=5에 이르러 occurrence를 찾는 상태" width="30%"}
:::
...
::: {.callout-note title="핵심 정리: 세 기법이 이용하는 정보"}
...
![`cad`를 5진법 $(2,0,3)_5$로 인코딩하면 $2\cdot5^2+0\cdot5+3=53$이다.](../figures/09-string-matching/03-numeric-encoding-cad-step2.svg){fig-alt="cad를 5진법 자리값으로 표현해 2*25+0*5+3=53을 계산한 상태" width="25%"}

![$P=$`abe`(value 9)와 $window=$`eec`(value 122, $122\bmod113=9$)는 문자열이 다른데도 해시가 둘 다 9로 같아 candidate가 되지만, 실제로는 `abe`$\ne$`eec`인 spurious hit이다.](../figures/09-string-matching/04-hash-equality-insufficient.svg){fig-alt="P=abe, value=9, h(P)=9인 박스와 window=eec, value=122, h=122 mod 113=9인 박스가 candidate 화살표로 이어지고, 아래 note에 두 문자열이 달라 spurious hit임이 표시된 다이어그램" width="80%"}
...
1. 예. 
2. 반드시 그렇지 않다. 
3. 실제 문자 verification.
:::

## Part E. Rolling Hash

$$value(x)=x[0]\cdot base^{m-1}+\cdots+x[m-1],\qquad v\gets0;\ v\gets base\cdot v+code(c)\ \text{for each }c$$
...
![현재 해시에서 맨 앞 문자의 기여분을 뺀다.](../figures/09-string-matching/05-rolling-update-derivation-step1.svg){fig-alt="state box에 current-leading×h 식이 표시되고 note에 leading contribution 제거라고 쓰인 상태" width="30%"}

## 2단계

![남은 digit들을 한 자리씩 왼쪽으로 밀어 자릿수를 맞춘다.](../figures/09-string-matching/05-rolling-update-derivation-step2.svg){fig-alt="state box가 base×(current-leading×h) 식으로 갱신되고 note에 남은 digit을 왼쪽으로 shift한다고 쓰인 상태" width="30%"}

## 3단계

![새로 들어온 마지막 문자를 더해 다음 window의 해시를 얻는다.](../figures/09-string-matching/05-rolling-update-derivation-step3.svg){fig-alt="state box에 next=base×(current-leading×h)+trailing 식이 표시되고 note에 새 trailing character 추가라고 쓰인 상태" width="45%"}
:::

![window가 한 칸 움직일 때 old 기여 제거 → ×base → new 문자 추가로 이어지는 정적 흐름도.](../figures/09-string-matching/06-rolling-update-flow.svg){fig-alt="old T[s] 기여 제거, x base, new T[s+m] 추가 세 상자가 화살표로 이어진 흐름도" width="60%"}

이전 window value와 미리 계산한 $h=base^{m-1}$만 있으면 재사용할 수 있다.
...
::: {.callout-note collapse="true" title="핵심 정리: window update 비용"}
...
modulus는 **저장되는 hash 값**을 $0,\ldots,q-1$로 제한한다. intermediate multiplication/subtraction은 구현에서 별도로 overflow를 피해야 한다. $q$가 작을수록 collision 가능성이 커지며 $base,q$와 input 가정이 expected cost에 영향을 준다.

$$h=base^{m-1}\bmod q,\qquad next=\bigl(base\cdot(current-code(T[s])h)+code(T[s+m])\bigr)\bmod q$$
...
![$h(P)=h(W)$여도 candidate일 뿐, $P\ne W$면 collision(spurious hit)이다.](../figures/09-string-matching/07-collision-is-candidate-step2.svg){fig-alt="hash가 같아 candidate로 표시됐지만 실제로는 문자열이 달라 collision으로 확정된 상태" width="35%"}
...
![$s=6$에서 $tHash=pHash=63$인 candidate가 문자 5개 모두 일치해 occurrence 6으로 확정된다.](../figures/09-string-matching/08-candidate-to-valid-hit-step2.svg){fig-alt="s=6에서 hash가 일치한 candidate가 eeaab 대 eeaab 검증을 거쳐 occurrence로 확정된 상태" width="45%"}
...
**구현**: `T="acebbceeaabceedb"`, `P="eeaab"`, base $5$, modulus $q=113$ 고정 예제로 검증한다.
...
::: {.callout-note collapse="true" title="핵심 정리: KMP를 택하는 경우"}
...
![`BAAB`: border `B`, LPS 1.](../figures/09-string-matching/09-prefix-lps-discovery-step1.svg){fig-alt="BAAB prefix에서 border B를 찾아 LPS 1을 확정하는 상태" width="55%"}

## 2단계

![`BAABA`: border `BA`, LPS 2.](../figures/09-string-matching/09-prefix-lps-discovery-step2.svg){fig-alt="BAABA prefix에서 border BA를 찾아 LPS 2를 확정하는 상태" width="55%"}

## 3단계

![prefix `BAABABAA`의 가장 긴 border는 `BAA`(LPS 3).](../figures/09-string-matching/09-prefix-lps-discovery-step3.svg){fig-alt="BAABABAA prefix에서 border BAA를 찾아 LPS 3을 확정하는 상태" width="55%"}
:::
...
![old $j=5$: `BAABA` matched, 현재 $T[i]=$`X`에서 mismatch.](../figures/09-string-matching/10-mismatch-fallback-trace-step1.svg){fig-alt="BAABA까지 매칭된 뒤 T[i]=X에서 mismatch가 난 상태" width="65%"}

## 2단계

![$lps[4]=2$: matched text suffix `BA` = pattern prefix `BA`.](../figures/09-string-matching/10-mismatch-fallback-trace-step2.svg){fig-alt="lps[4]=2로 matched suffix BA와 pattern prefix BA가 같음을 보이는 상태" width="65%"}

## 3단계

![new $j=2$, 같은 text index $i=5$: equal parts를 정렬.](../figures/09-string-matching/10-mismatch-fallback-trace-step3.svg){fig-alt="new j=2로 재정렬하되 text index i=5는 그대로인 상태" width="65%"}

## 4단계

![$j\gets lps[4]=2$로 border `BA`를 재사용한 뒤, 같은 text index에서 $T[i]=$`X`를 $P[2]=$`A`와 재비교한다 — $i$는 감소하지 않는다.](../figures/09-string-matching/10-mismatch-fallback-trace-step4.svg){fig-alt="lps[4]=2가 가리키는 border BA에 패턴을 맞추고 같은 text index에서 비교를 재개하는 상태" width="65%"}
:::
...
KMP의 fallback은 두 가지 convention으로 구현된다. 이 장이 쓰는 standard LPS는 $lps[i]$에 border 길이를 담고($lps[0]=0$, mismatch 시 $j\gets lps[j-1]$), 고전 문헌의 failure function은 $failure[i]=lps[i]-1$로 정의해 sentinel($failure[0]=-1$, mismatch 시 $j\gets failure[j]$)을 쓴다. 아래 표는 같은 정보의 두 표현을 대조한다.
...
본문은 standard LPS($lps[0]=0$) convention만 쓴다. sentinel failure($failure[0]=-1$) convention은 부록에서만 참고로 다룬다.
:::
...
![$i=1,2$: $A\ne B\Rightarrow lps[1]=lps[2]=0$. `[0,0,0]`.](../figures/09-string-matching/11-lps-construction-trace-step1.svg){fig-alt="i=1,2에서 mismatch로 lps[1]=lps[2]=0이 되어 [0,0,0]까지 채워진 상태" width="70%"}

## 2단계

![$i=3$: $B=B\Rightarrow lps[3]=1$. `[0,0,0,1]`.](../figures/09-string-matching/11-lps-construction-trace-step2.svg){fig-alt="i=3에서 매칭되어 lps[3]=1, [0,0,0,1]까지 채워진 상태" width="70%"}

## 3단계

![$i=4$: $A=A\Rightarrow lps[4]=2$. `[0,0,0,1,2]`.](../figures/09-string-matching/11-lps-construction-trace-step3.svg){fig-alt="i=4에서 매칭되어 lps[4]=2, [0,0,0,1,2]까지 채워진 상태" width="70%"}

## 4단계

![$i=5$: $B\ne A$, $len:2\to0$, $B=B\Rightarrow1$. `[0,0,0,1,2,1]`.](../figures/09-string-matching/11-lps-construction-trace-step4.svg){fig-alt="i=5에서 len이 2에서 0으로 fallback한 뒤 매칭되어 lps[5]=1, [0,0,0,1,2,1]까지 채워진 상태" width="70%"}

## 5단계

![$i=6$: $A=A\Rightarrow2$. `[0,0,0,1,2,1,2]`.](../figures/09-string-matching/11-lps-construction-trace-step5.svg){fig-alt="i=6에서 매칭되어 lps[6]=2, [0,0,0,1,2,1,2]까지 채워진 상태" width="70%"}

## 6단계

![$i=7$에서 $P[7]=$`A`$=P[2]$로 $len=3$ → $lps[7]=3$, 최종 LPS `[0,0,0,1,2,1,2,3]` 완성.](../figures/09-string-matching/11-lps-construction-trace-step6.svg){fig-alt="BAABABAA의 각 인덱스마다 lps 값을 채워나가 최종적으로 0,0,0,1,2,1,2,3 표를 완성하는 상태" width="70%"}
:::
...
![$i=4,j=4$에서 `X`≠`A` mismatch. 이미 맞은 `BAAB`의 정보를 버리지 않는다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step1.svg){fig-alt="i=4,j=4에서 X와 A가 mismatch로 표시된 상태" width="70%"}

## 2단계

![$j\gets lps[3]=1$: 길이 1의 border만 재사용하고 같은 `X`를 비교한다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step2.svg){fig-alt="j가 lps[3]=1로 fallback해 같은 X를 다시 비교하는 상태" width="70%"}

## 3단계

![다시 mismatch이므로 $j\gets lps[0]=0$. text index $i$는 아직 4다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step3.svg){fig-alt="다시 mismatch가 나 j가 lps[0]=0으로 fallback하지만 text index i=4는 그대로인 상태" width="70%"}

## 4단계

![$j=0$ mismatch 뒤 $i\gets5$. 새 alignment의 첫 문자가 일치한다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step4.svg){fig-alt="j=0에서 mismatch 후 i=5로 이동해 새 alignment의 첫 문자가 일치하는 상태" width="70%"}

## 5단계

![$i\gets5$ 이후 shift 5에서 $P=$`BAABABAA` 전체가 매칭돼 occurrence 5로 확정된다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step5.svg){fig-alt="shift 5에서 패턴 BAABABAA가 text와 전부 일치해 occurrence가 확정된 상태" width="70%"}
:::
...
![위치 0에서 AAA가 match된다. 다음 탐색을 위해 j를 lps[2]=2로 되돌린다.](../figures/09-string-matching/13-kmp-overlapping-match-step1.svg){fig-alt="match at 0; j←lps[2]=2라는 문구가 표시된 상태" width="55%"}

## 2단계

![되돌린 j 덕분에 overlap을 잃지 않고 이어서 탐색해 matches [0,1,2]를 모두 찾는다.](../figures/09-string-matching/13-kmp-overlapping-match-step2.svg){fig-alt="continue ⇒ matches [0,1,2]라는 문구가 표시된 상태" width="55%"}
:::
...
::: {.callout-warning collapse="true" title="주의: preprocessing 비용 표기"}
search만 $\Theta(n)$이라 말할 때도 preprocessing $\Theta(m)$을 함께 명시한다.
:::

## Part N. Boyer-Moore의 아이디어

이 장은 full Boyer-Moore가 아니라, bad-character rule만 쓰는 실용적 단순화인 **Boyer-Moore-Horspool**을 다룬다. good-suffix rule을 포함한 full Boyer-Moore는 Gusfield, *Algorithms on Strings, Trees, and Sequences*의 Boyer-Moore 장을 참고하라.
...
![shift 0 정렬에서 오른쪽 끝 G와 R을 먼저 비교한다 — 이 한 번의 mismatch가 shift 거리의 근거가 된다.](../figures/09-string-matching/14-right-to-left-comparison-step1.svg){fig-alt="text CATIG와 pattern TIGER를 shift 0으로 정렬하고 오른쪽 끝 G와 R이 빨강으로 강조되어 mismatch를 표시한 그림" width="70%"}

## 2단계

![shift 1에서는 오른쪽 끝 R부터 왼쪽으로 다섯 문자가 모두 일치한다.](../figures/09-string-matching/14-right-to-left-comparison-step2.svg){fig-alt="text XTIGER에 pattern TIGER를 shift 1로 정렬해 다섯 문자가 모두 초록으로 일치 표시되고 비교 방향이 right에서 left로 표기된 그림" width="70%"}
:::
...
![$j=0,1,2$ 계산: $shift[T]=4,\ shift[I]=3,\ shift[G]=2$.](../figures/09-string-matching/15-tiger-shift-table-step1.svg){fig-alt="TIGER shift table에서 T=4,I=3,G=2가 계산되어 채워지고 E,R은 물음표로 미확정이며 other=5 초기화값 배지가 함께 있는 상태" width="55%"}

## 2단계

![$j=3$(loop 마지막) 계산: $shift[E]=1$이 채워진다.](../figures/09-string-matching/15-tiger-shift-table-step2.svg){fig-alt="TIGER shift table에 shift[E]=1이 채워지고 R만 물음표로 남은 상태" width="55%"}

## 3단계

![`R`(index $4=m-1$)은 $j=0..m-2$ loop에서 제외되어 계산되지 않고 초기화 default $5$에 남는다 — 옅은 색으로 계산된 값과 구분한다.](../figures/09-string-matching/15-tiger-shift-table-step3.svg){fig-alt="TIGER shift table에서 T=4,I=3,G=2,E=1은 초록으로 계산되어 표시되고 R=5는 옅은 색으로 default임을 구분해 표시" width="55%"}
:::

마지막 $R$은 preprocessing loop에서 제외한다.

![window 마지막 문자가 pattern에 없는 `b`이면 $shift[b]=5$로 window 전체를 건너뛴다.](../figures/09-string-matching/16-absent-character-shift-step2.svg){fig-alt="TIGER 패턴에 없는 문자 b가 window 끝에 오면 shift 5로 전체 window를 건너뛰는 상태" width="55%"}
...
full algorithm은 bad-character shift와 good-suffix shift를 모두 계산하고 안전한 더 큰 shift를 사용한다. Part N에서 밝혔듯 이 장의 구현과 트레이스는 good-suffix table이 없는 **Boyer-Moore-Horspool**이다.
:::
...
![shift table은 마지막 문자 `L`을 제외하고 만든다.](../figures/09-string-matching/17-rational-repeated-character-step1.svg){fig-alt="RATIONAL에서 마지막 문자 L이 전처리 대상에서 제외됨을 보여주는 상태" width="70%"}

## 2단계

![같은 문자 중 가장 오른쪽 occurrence를 사용한다 — 오른쪽 occurrence가 더 작은 안전한 shift를 준다.](../figures/09-string-matching/17-rational-repeated-character-step2.svg){fig-alt="왼쪽 A(index 1)와 오른쪽 A(index 6) 중 오른쪽 occurrence를 사용하기로 표시하는 상태" width="70%"}

## 3단계

![더 오른쪽 A(index 6)가 왼쪽 A를 덮어써 $shift[A]=7-6=1$로 확정된다.](../figures/09-string-matching/17-rational-repeated-character-step3.svg){fig-alt="RATIONAL의 두 A 중 더 오른쪽 A를 사용해 shift[A]=1을 확정하는 상태" width="70%"}
:::
...
![비교는 pattern의 마지막 문자부터 시작한다.](../figures/09-string-matching/18-horspool-trace-tiger-step1.svg){fig-alt="CATIGER 위에서 TIGER의 마지막 문자 R과 G가 mismatch로 표시된 상태" width="70%"}

## 2단계

![pattern 안의 마지막 `G`가 window 끝으로 오도록 이동한다(shift 2).](../figures/09-string-matching/18-horspool-trace-tiger-step2.svg){fig-alt="pattern의 마지막 G가 window 끝에 오도록 shift 2로 이동하는 상태" width="70%"}

## 3단계

![새 window에서 오른쪽부터 왼쪽으로 모든 문자를 검증한다.](../figures/09-string-matching/18-horspool-trace-tiger-step3.svg){fig-alt="새 window에서 오른쪽부터 왼쪽으로 T,I,G,E,R 순서로 전체 문자를 검증하는 상태" width="70%"}

## 4단계

![shift 2로 이동한 뒤 오른쪽부터 왼쪽으로 `TIGER` 전체가 `T`와 일치한다 — match.](../figures/09-string-matching/18-horspool-trace-tiger-step4.svg){fig-alt="CATIGER에서 TIGER 끝 문자 mismatch 후 shift 2로 이동해 오른쪽부터 왼쪽으로 전체 문자를 검증하는 상태" width="70%"}
:::
...
::: {.callout-note collapse="true" title="핵심 정리: Horspool의 shift 폭"}
...
테스트 행렬은 empty/$m>n$/no/begin/end/overlap/repeated 경우, 의도적 collision과 negative rolling 식, 4개 고정 LPS table(연속 fallback chain 포함), TIGER/RATIONAL table과 긴 mismatch progress regression, Java UTF-16 code-unit·C UTF-8 byte-offset 예제, 언어당 고정 seed 5,000-case random test, C의 ASan/UBSan bounds check를 포함한다.
...
1. 아니다. 
2. 직접 character/byte 비교. 
3. Java code unit과 C byte로 다르다.
:::
...
modulus 없이 $base^m$ 크기의 정수를 그대로 유지하면 digit 수가 $m$과 함께 증가한다. 이때 곱셈·덧셈을 unit-cost로 보는 $O(1)$ rolling update는 bit complexity 관점에서 성립하지 않을 수 있다 — 수학적 modular arithmetic, fixed-width machine implementation의 intermediate overflow, arbitrary-precision bit complexity는 서로 다른 문제다.
