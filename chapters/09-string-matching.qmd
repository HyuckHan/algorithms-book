...
![$P=$`abe`(value 9)와 $window=$`eec`(value 122, $122\bmod113=9$)는 문자열이 다른데도 해시가 둘 다 9로 같아 candidate가 되지만, 실제로는 `abe`$\ne$`eec`인 spurious hit이다.](../figures/09-string-matching/04-hash-equality-insufficient.svg){fig-alt="P=abe, value=9, h(P)=9인 박스와 window=eec, value=122, h=122 mod 113=9인 박스가 candidate 화살표로 이어지고, 아래 note에 두 문자열이 달라 spurious hit임이 표시된 다이어그램" width="50%"}
...
![남은 digit들을 한 자리씩 왼쪽으로 밀어 자릿수를 맞춘다.](../figures/09-string-matching/05-rolling-update-derivation-step2.svg){fig-alt="state box가 d(current-leading×h) 식으로 갱신되고 note에 남은 digit을 왼쪽으로 shift한다고 쓰인 상태" width="30%"}

## 3단계

![새로 들어온 마지막 문자를 더해 다음 window의 해시를 얻는다.](../figures/09-string-matching/05-rolling-update-derivation-step3.svg){fig-alt="state box에 next=d(current-leading×h)+trailing 식이 표시되고 note에 새 trailing character 추가라고 쓰인 상태" width="45%"}
:::
