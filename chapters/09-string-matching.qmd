...
![$s=0$: `P`의 `AAAAA`는 일치하지만 $P[5]=$`B`가 $T[5]=$`A`와 mismatch — 한 shift에 5 match + 1 mismatch.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step1.svg){fig-alt="AAAAAAAAAAB에서 s=0으로 맞춘 AAAAAB가 5문자까지 일치하다가 6번째에서 mismatch가 나는 1단계 상태" width="30%"}

## 2단계

![$s=1$: 앞서 본 A들을 다시 비교한다 — $s=1$부터 $s=4$까지 매 shift가 5개의 A를 재확인하고 B에서 mismatch.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step2.svg){fig-alt="s=1로 이동해 같은 긴 prefix를 다시 비교하는 2단계 상태" width="30%"}

## 3단계

![$s=5$: 5개의 A에 이어 마지막 B까지 일치 — occurrence 5.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step3.svg){fig-alt="AAAAA 반복 뒤 mismatch가 나는 최악 사례에서 s=5에 이르러 occurrence를 찾는 3단계 트레이스의 마지막 상태" width="30%"}
:::
...
![1. leading contribution 제거: $current-leading\cdot h$.](../figures/09-string-matching/05-rolling-update-derivation-step1.svg){fig-alt="current에서 leading 항을 뺀 current-leading×h 식을 보여주는 1단계 상태" width="30%"}

## 2단계

![2. 남은 digits를 왼쪽으로 shift: $d(current-leading\cdot h)$.](../figures/09-string-matching/05-rolling-update-derivation-step2.svg){fig-alt="leading 항을 뺀 값에 d를 곱해 남은 digit을 왼쪽으로 옮기는 2단계 상태" width="30%"}

## 3단계

![rolling update 유도 3단계: leading contribution 제거 → 남은 digit을 왼쪽으로 shift → 새 trailing character 추가.](../figures/09-string-matching/05-rolling-update-derivation-step3.svg){fig-alt="current에서 leading 항을 빼고 d를 곱한 뒤 trailing 문자를 더해 next 값을 얻는 3단계 트레이스의 마지막 상태" width="45%"}
:::
...
![`BAAB`: border `B`, LPS 1.](../figures/09-string-matching/09-prefix-lps-discovery-step1.svg){fig-alt="BAAB prefix에서 border B를 찾아 LPS 1을 확정하는 1단계 상태" width="55%"}

## 2단계

![`BAABA`: border `BA`, LPS 2.](../figures/09-string-matching/09-prefix-lps-discovery-step2.svg){fig-alt="BAABA prefix에서 border BA를 찾아 LPS 2를 확정하는 2단계 상태" width="55%"}

## 3단계

![prefix `BAABABAA`의 가장 긴 border는 `BAA`(LPS 3).](../figures/09-string-matching/09-prefix-lps-discovery-step3.svg){fig-alt="BAAB, BAABA, BAABABAA 세 prefix에서 각각 border를 찾아 LPS 1,2,3을 확정하는 3단계 트레이스의 마지막 상태" width="55%"}
:::
...
![old $j=5$: `BAABA` matched, 현재 $T[i]=$`X`에서 mismatch.](../figures/09-string-matching/10-mismatch-fallback-trace-step1.svg){fig-alt="BAABA까지 매칭된 뒤 T[i]=X에서 mismatch가 난 1단계 상태" width="65%"}

## 2단계

![$lps[4]=2$: matched text suffix `BA` = pattern prefix `BA`.](../figures/09-string-matching/10-mismatch-fallback-trace-step2.svg){fig-alt="lps[4]=2로 matched suffix BA와 pattern prefix BA가 같음을 보이는 2단계 상태" width="65%"}

## 3단계

![new $j=2$, 같은 text index $i=5$: equal parts를 정렬.](../figures/09-string-matching/10-mismatch-fallback-trace-step3.svg){fig-alt="new j=2로 재정렬하되 text index i=5는 그대로인 3단계 상태" width="65%"}

## 4단계

![$j\gets lps[4]=2$로 border `BA`를 재사용한 뒤, 같은 text index에서 $T[i]=$`X`를 $P[2]=$`A`와 재비교한다 — $i$는 감소하지 않는다.](../figures/09-string-matching/10-mismatch-fallback-trace-step4.svg){fig-alt="BAABA까지 매칭된 뒤 X에서 mismatch가 나 lps[4]=2인 border BA로 fallback해 같은 text index에서 재비교하는 4단계 트레이스의 마지막 상태" width="65%"}
:::
...
![$i=1,2$: $A\ne B\Rightarrow lps[1]=lps[2]=0$. `[0,0,0]`.](../figures/09-string-matching/11-lps-construction-trace-step1.svg){fig-alt="i=1,2에서 mismatch로 lps[1]=lps[2]=0이 되어 [0,0,0]까지 채워진 1단계 상태" width="70%"}

## 2단계

![$i=3$: $B=B\Rightarrow lps[3]=1$. `[0,0,0,1]`.](../figures/09-string-matching/11-lps-construction-trace-step2.svg){fig-alt="i=3에서 매칭되어 lps[3]=1, [0,0,0,1]까지 채워진 2단계 상태" width="70%"}

## 3단계

![$i=4$: $A=A\Rightarrow lps[4]=2$. `[0,0,0,1,2]`.](../figures/09-string-matching/11-lps-construction-trace-step3.svg){fig-alt="i=4에서 매칭되어 lps[4]=2, [0,0,0,1,2]까지 채워진 3단계 상태" width="70%"}

## 4단계

![$i=5$: $B\ne A$, $len:2\to0$, $B=B\Rightarrow1$. `[0,0,0,1,2,1]`.](../figures/09-string-matching/11-lps-construction-trace-step4.svg){fig-alt="i=5에서 len이 2에서 0으로 fallback한 뒤 매칭되어 lps[5]=1, [0,0,0,1,2,1]까지 채워진 4단계 상태" width="70%"}

## 5단계

![$i=6$: $A=A\Rightarrow2$. `[0,0,0,1,2,1,2]`.](../figures/09-string-matching/11-lps-construction-trace-step5.svg){fig-alt="i=6에서 매칭되어 lps[6]=2, [0,0,0,1,2,1,2]까지 채워진 5단계 상태" width="70%"}

## 6단계

![$i=7$에서 $P[7]=$`A`$=P[2]$로 $len=3$ → $lps[7]=3$, 최종 LPS `[0,0,0,1,2,1,2,3]` 완성.](../figures/09-string-matching/11-lps-construction-trace-step6.svg){fig-alt="BAABABAA의 각 인덱스마다 lps 값을 채워나가 최종적으로 0,0,0,1,2,1,2,3 표를 완성하는 6단계 트레이스의 마지막 상태" width="70%"}
:::
...
![$i=4,j=4$에서 `X`≠`A` mismatch. 이미 맞은 `BAAB`의 정보를 버리지 않는다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step1.svg){fig-alt="i=4,j=4에서 X와 A가 mismatch로 표시된 1단계 상태" width="70%"}

## 2단계

![$j\gets lps[3]=1$: 길이 1의 border만 재사용하고 같은 `X`를 비교한다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step2.svg){fig-alt="j가 lps[3]=1로 fallback해 같은 X를 다시 비교하는 2단계 상태" width="70%"}

## 3단계

![다시 mismatch이므로 $j\gets lps[0]=0$. text index $i$는 아직 4다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step3.svg){fig-alt="다시 mismatch가 나 j가 lps[0]=0으로 fallback하지만 text index i=4는 그대로인 3단계 상태" width="70%"}

## 4단계

![$j=0$ mismatch 뒤 $i\gets5$. 새 alignment의 첫 문자가 일치한다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step4.svg){fig-alt="j=0에서 mismatch 후 i=5로 이동해 새 alignment의 첫 문자가 일치하는 4단계 상태" width="70%"}

## 5단계

![$i\gets5$ 이후 shift 5에서 $P=$`BAABABAA` 전체가 매칭돼 occurrence 5로 확정된다.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step5.svg){fig-alt="BAABX 뒤 mismatch가 나 lps 값을 따라 두 번 fallback한 뒤 shift 5에서 BAABABAA 전체가 매칭되는 5단계 트레이스의 마지막 상태" width="70%"}
:::
...
![오른쪽 suffix부터 왼쪽으로 비교해, 끝 문자 mismatch만으로도 큰 shift의 근거를 얻는다.](../figures/09-string-matching/14-right-to-left-comparison-step2.svg){fig-alt="text와 pattern을 오른쪽 끝부터 왼쪽으로 비교하는 방향과 mismatch 시 문자·suffix 정보를 사용한다는 설명이 표시된 상태" width="70%"}
...
![$j=0,1,2$ 계산: $shift[T]=4,\ shift[I]=3,\ shift[G]=2$.](../figures/09-string-matching/15-tiger-shift-table-step1.svg){fig-alt="TIGER shift table에서 T=4,I=3,G=2가 계산되어 채워지고 E,R은 아직 물음표로 미확정인 1단계 상태" width="55%"}

## 2단계

![$j=3$(loop 마지막) 계산: $shift[E]=1$ 추가. 나머지 문자의 초기화값 $shift[\text{other}]=m=5$도 함께 표시.](../figures/09-string-matching/15-tiger-shift-table-step2.svg){fig-alt="TIGER shift table에 shift[E]=1이 추가되고 other=5 초기화값 배지가 함께 표시된 2단계 상태" width="55%"}

## 3단계

![`R`(index $4=m-1$)은 $j=0..m-2$ loop에서 제외되어 계산되지 않고 초기화 default $5$에 남는다 — 옅은 색으로 계산된 값과 구분한다.](../figures/09-string-matching/15-tiger-shift-table-step3.svg){fig-alt="TIGER shift table 최종 상태: T=4,I=3,G=2,E=1은 초록으로 계산되어 표시되고 R=5는 옅은 색으로 default임을 구분해 표시" width="55%"}
:::
...
![shift table은 마지막 문자 `L`을 제외하고 만든다.](../figures/09-string-matching/17-rational-repeated-character-step1.svg){fig-alt="RATIONAL에서 마지막 문자 L이 전처리 대상에서 제외됨을 보여주는 1단계 상태" width="70%"}

## 2단계

![같은 문자 중 가장 오른쪽 occurrence를 사용한다 — 오른쪽 occurrence가 더 작은 안전한 shift를 준다.](../figures/09-string-matching/17-rational-repeated-character-step2.svg){fig-alt="왼쪽 A(index 1)와 오른쪽 A(index 6) 중 오른쪽 occurrence를 사용하기로 표시하는 2단계 상태" width="70%"}

## 3단계

![더 오른쪽 A(index 6)가 왼쪽 A를 덮어써 $shift[A]=7-6=1$로 확정된다.](../figures/09-string-matching/17-rational-repeated-character-step3.svg){fig-alt="RATIONAL의 두 A 중 더 오른쪽 A를 사용해 shift[A]=1을 확정하는 3단계 트레이스의 마지막 상태" width="70%"}
:::
...
![비교는 pattern의 마지막 문자부터 시작한다.](../figures/09-string-matching/18-horspool-trace-tiger-step1.svg){fig-alt="CATIGER 위에서 TIGER의 마지막 문자 R과 G가 mismatch로 표시된 1단계 상태" width="70%"}

## 2단계

![pattern 안의 마지막 `G`가 window 끝으로 오도록 이동한다(shift 2).](../figures/09-string-matching/18-horspool-trace-tiger-step2.svg){fig-alt="pattern의 마지막 G가 window 끝에 오도록 shift 2로 이동하는 2단계 상태" width="70%"}

## 3단계

![새 window에서 오른쪽부터 왼쪽으로 모든 문자를 검증한다.](../figures/09-string-matching/18-horspool-trace-tiger-step3.svg){fig-alt="새 window에서 오른쪽부터 왼쪽으로 T,I,G,E,R 순서로 전체 문자를 검증하는 3단계 상태" width="70%"}

## 4단계

![shift 2로 이동한 뒤 오른쪽부터 왼쪽으로 `TIGER` 전체가 `T`와 일치한다 — match.](../figures/09-string-matching/18-horspool-trace-tiger-step4.svg){fig-alt="CATIGER에서 TIGER 끝 문자 mismatch 후 shift 2로 이동해 오른쪽부터 왼쪽으로 전체 문자를 검증하는 4단계 트레이스의 마지막 상태" width="70%"}
:::
