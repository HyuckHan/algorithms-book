...
![Naive alignment trace: $s=0$에서 $C\ne A$ mismatch → $s=1$에서 비교 시작 → $A=B=A$ prefix 확인 → match 확정까지 4단계.](../figures/09-string-matching/01-naive-alignment-trace-step4.svg){fig-alt="C,A,B,A,B,A,C 텍스트 위에서 s=1에 A,B,A 패턴을 맞춰 세 문자가 모두 일치해 match로 확정되는 4단계 트레이스의 마지막 상태" width="45%"}
...
![$s=0$: AAAAA 뒤 $A\ne B$ — 5 match + 1 mismatch.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step1.svg){fig-alt="AAAAAAAAAAB에서 s=0으로 맞춘 AAAAAB가 5문자까지 일치하다가 6번째에서 mismatch가 나는 1단계 상태" width="30%"}

## 2단계

![$s=1$: 같은 긴 prefix를 재비교 — 이미 본 text 문자를 다시 비교한다.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step2.svg){fig-alt="s=1로 이동해 같은 긴 prefix를 다시 비교하는 2단계 상태" width="30%"}

## 3단계

![`T=AAAAAAAAAAB`, `P=AAAAAB`에서 각 shift가 5개의 A를 재확인하다가 $s=5$에서 match하는 최악 예 3단계.](../figures/09-string-matching/02-naive-worst-case-repeated-prefix-step3.svg){fig-alt="AAAAA 반복 뒤 mismatch가 나는 최악 사례에서 s=5에 이르러 occurrence를 찾는 3단계 트레이스의 마지막 상태" width="30%"}
:::
...
![`cad`를 5진법 자리값 $(2,0,3)_5$로 인코딩해 $2\cdot5^2+0\cdot5+3=53$을 계산하는 2단계.](../figures/09-string-matching/03-numeric-encoding-cad-step2.svg){fig-alt="cad를 5진법 자리값으로 표현한 뒤 2*25+0*5+3=53을 계산하는 2단계 트레이스의 마지막 상태" width="25%"}
...
![$h(P)=h(W)$이면 candidate일 뿐이고, $P\ne W$로 판명되면 collision(spurious hit)임을 보여주는 2단계.](../figures/09-string-matching/07-collision-is-candidate-step2.svg){fig-alt="hash가 같아 candidate로 표시된 뒤 실제로는 문자열이 달라 collision으로 확정되는 2단계 트레이스의 마지막 상태" width="35%"}
...
![$s=6$에서 $tHash=pHash=63$인 candidate가 실제 문자 5개 모두 일치해 occurrence 6으로 확정되는 2단계.](../figures/09-string-matching/08-candidate-to-valid-hit-step2.svg){fig-alt="s=6에서 hash가 일치한 candidate가 eeaab 대 eeaab 검증을 거쳐 occurrence로 확정되는 2단계 트레이스의 마지막 상태" width="45%"}
...
![`BAAB`(border B, LPS 1) → `BAABA`(border BA, LPS 2) → `BAABABAA`(border BAA, LPS 3)로 이어지는 3단계.](../figures/09-string-matching/09-prefix-lps-discovery-step3.svg){fig-alt="BAAB, BAABA, BAABABAA 세 prefix에서 각각 border를 찾아 LPS 1,2,3을 확정하는 3단계 트레이스의 마지막 상태" width="55%"}
:::
...
![old $j=5$: `BAABA` matched, `X` mismatch → $lps[4]=2$로 border `BA` 재사용 → new $j=2$, 같은 text index $i=5$ → `X`를 $P[2]$와 재비교하는 4단계.](../figures/09-string-matching/10-mismatch-fallback-trace-step4.svg){fig-alt="BAABA까지 매칭된 뒤 X에서 mismatch가 나 lps[4]=2인 border BA로 fallback해 같은 text index에서 재비교하는 4단계 트레이스의 마지막 상태" width="65%"}
:::
...
![`BAABABAA`의 LPS를 채우는 과정: $i=1,2$ mismatch → $i=3$ 매칭 → ... → $i=7$까지 6단계 만에 `[0,0,0,1,2,1,2,3]` 완성.](../figures/09-string-matching/11-lps-construction-trace-step6.svg){fig-alt="BAABABAA의 각 인덱스마다 lps 값을 채워나가 최종적으로 0,0,0,1,2,1,2,3 표를 완성하는 6단계 트레이스의 마지막 상태" width="70%"}
:::
...
![$i=4,j=4$에서 X≠A mismatch → $j\gets lps[3]=1$ → 다시 mismatch, $j\gets lps[0]=0$ → $i\gets5$ → shift 5에서 전체 매칭 완료까지 5단계.](../figures/09-string-matching/12-kmp-mismatch-recovery-trace-step5.svg){fig-alt="BAABX 뒤 mismatch가 나 lps 값을 따라 두 번 fallback한 뒤 shift 5에서 BAABABAA 전체가 매칭되는 5단계 트레이스의 마지막 상태" width="70%"}
:::
...
![`CATIG` 위에 `TIGER`를 놓고 끝 문자 G/R mismatch만으로 큰 shift의 근거를 얻은 뒤, 오른쪽에서 왼쪽으로 비교해 전체가 일치하는 2단계.](../figures/09-string-matching/14-right-to-left-comparison-step2.svg){fig-alt="CATIG 위에 TIGER를 놓고 끝 문자부터 비교해 mismatch 근거를 얻은 뒤 이동해 전체 일치를 확인하는 2단계 트레이스의 마지막 상태" width="70%"}
...
![`TIGER`의 shift table: $T=4,I=3,G=2$까지 계산하고 $E=1,R=5,\text{other}=5$까지 마무리하는 2단계.](../figures/09-string-matching/15-tiger-shift-table-step2.svg){fig-alt="TIGER 각 문자의 shift 값을 하나씩 계산해 T=4,I=3,G=2,E=1,R=5,other=5를 완성하는 2단계 트레이스의 마지막 상태" width="45%"}

마지막 $R$은 preprocessing loop에서 제외한다.

![`P=TIGER`에서 window 마지막 문자가 `b`(pattern에 없음)이면 $shift[b]=5$로 window 전체를 건너뛴다.](../figures/09-string-matching/16-absent-character-shift-step2.svg){fig-alt="TIGER 패턴에 없는 문자 b가 window 끝에 오면 shift 5로 전체 window를 건너뛰는 2단계 트레이스의 마지막 상태" width="55%"}
...
![`RATIONAL`에서 반복되는 `A`: 왼쪽 A를 먼저 표시했다가, 더 오른쪽 A(index 6)로 덮어써 $shift[A]=7-6=1$을 확정하는 3단계.](../figures/09-string-matching/17-rational-repeated-character-step3.svg){fig-alt="RATIONAL의 두 A 중 더 오른쪽 A를 사용해 shift[A]=1을 확정하는 3단계 트레이스의 마지막 상태" width="70%"}
:::
...
![`CATIGER`에서 `TIGER`의 끝 G/R mismatch 뒤 shift 2로 이동 → 오른쪽부터 전체 검증까지 4단계.](../figures/09-string-matching/18-horspool-trace-tiger-step4.svg){fig-alt="CATIGER에서 TIGER 끝 문자 mismatch 후 shift 2로 이동해 오른쪽부터 왼쪽으로 전체 문자를 검증하는 4단계 트레이스의 마지막 상태" width="70%"}
:::
