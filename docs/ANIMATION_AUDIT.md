...
**복원 현황 갱신(2026-08-02)**: 이 감사를 근거로 L03/L04/L06/L07/L09/L10의 (A)/(B)/(C) findings를
tabset으로 복원하는 작업을 진행했으나, **L09·L10의 복원만 `git revert`로 되돌렸다** — 원본
(Codex 신규 서사) 대조 검증을 위해 보류 상태다(REVIEW_NOTES.md #13 참고). L01~L08(L03/L04/L06/
L07 포함)의 복원은 유지된다. L09/L10의 step SVG 파일 자체는 삭제하지 않고 디스크에 남아 있으므로,
검증 후 언제든 재복원할 수 있다. 아래 L09·L10 절의 "웹북 step SVG 수"/"qmd 삽입 수" 표는 **감사
당시(복원 전) 상태를 그대로 보존**한 기록이며, 현재 실제 qmd 상태와는 다시 일치한다(복원 전
상태로 되돌아갔으므로).
...
> **상태(2026-08-02): 보류.** 이 절의 16건은 한 차례 tabset으로 복원됐으나 원본 대조 검증을
> 위해 `git revert`(`11323f1`)로 되돌렸다 — qmd는 현재 아래 표와 같은 복원 전 상태다. step SVG는
> 유지되어 있어 검증 후 재복원 가능하다. REVIEW_NOTES.md #13 참고.
...
> **상태(2026-08-02): 보류.** 이 절의 13건은 한 차례 tabset으로 복원됐으나 원본 대조 검증을
> 위해 `git revert`(`699cafa`)로 되돌렸다 — qmd는 현재 아래 표와 같은 복원 전 상태다. step SVG는
> 유지되어 있어 검증 후 재복원 가능하다. REVIEW_NOTES.md #13 참고.
