...
| H3 | L09 `03-numeric-encoding-cad` "원본 28 오류" 문구 | 내용 | 원본 교정 노출(웹북 독립성 저해) | 문구 삭제, cad=53만. [방침] "원본 대비 교정" 노출 전반에 적용 — 정독하며 같은 유형(L09 0-based, L10 pick/AP/C구현) 수집. **[2026-08-07 확인]** 본체는 `94b8c3f`(text_patch `numeric_encoding_cad_drop_correction_note`)로 처리 완료. 후속 후보 2건도 이미 처리됨 확인 — "L09 0-based"는 원본 대비 서술이 아니라 챕터 자체의 설계 선언이었고, "L10 pick/AP/C구현"은 원본의 `alertblock{원본 교정}` 프레이밍이 웹북 번역에서 이미 벗겨지고 알고리즘 근거만 남아 있었다. `chapters/*.qmd` 전수 grep 결과 "원본 대비 교정" 노출 0건 | 완료 |
...
빌드 파이프라인(`scripts/extract_tikz.py`)과 dvisvgm 자체에서 반복적으로 나타나는, 콘텐츠와
무관한 도구 결함·함정을 기록한다. 개별 그림 수정 이력이 아니라 앞으로 같은 종류의 그림 작업을
할 때 미리 알아야 할 사항이므로 위 표 형식과 별도로 둔다.

### 1. dvisvgm `<defs>` 글리프 순서 비결정성

`compile_svg_verified`의 재시도 검사가 반복 실패할 수 있다. 원인은 dvisvgm이 `--no-fonts`로
글리프를 path화할 때 `<defs>` 안의 수집 순서가 비결정적인 것이다. 렌더 geometry는 매번 동일하고
바이트만 흔들린다. 같은 글리프가 한 state에 여러 번 등장할수록 확률이 올라간다.

**2026-08-06**: `10-graph-coloring-trace` step4가 `×` 3회 반복으로 5회 연속 실패해 수동 개입이
필요했다. `×`를 `\draw` 기반 `\rejectmark`로 바꿔 그 트리거는 제거했으나, 이후에도 다른 글리프
(`g2-82`, `g1-29`)에서 같은 현상이 재발했고 step3는 매 실행마다 3번째 시도를 요구한다. 숫자·한글
음절도 같은 위험군이므로 글리프를 개별 치환하는 방식으로는 근본 해결이 안 된다. 2/3 재시도
안전망이 실질적 완화책이며, 실패 시 재실행이 표준 대응이다.

### 2. 매크로는 tikzpicture 안에 정의해야 한다

`extract_tikz.py`는 tikzpicture 본문만 추출해 독립 컴파일한다. 프레임 밖이나 preamble에 정의한
매크로는 추출된 조각에서 보이지 않아 웹북 빌드만 깨진다(슬라이드는 정상). 그림 전용 매크로는
반드시 `\begin{tikzpicture}` 안에 두어라.

### 3. 주석에 `\begin{tikzpicture}` 문자열을 넣지 마라

추출 정규식은 주석을 구분하지 않는다. `.tex` 주석 안에 `\begin{tikzpicture}` 또는
`\end{tikzpicture}` 문자열이 그대로 들어가면 그 지점에서 그림이 잘리고, `overlay_steps()`가 빈
리스트를 반환해 `mode: sequence`가 조용히 무너진다. 예시를 주석에 쓸 때는 문자열을 변형하라(예:
`begin{tikzpicture}`처럼 백슬래시 생략).

### 4. 원본 lecture-notes의 자기 언급형 `alertblock{원본 교정}`

원본 lecture-notes에는 `alertblock{원본 교정}` 스타일의 자기 언급이 L06·L07·L08·L09·L10에
걸쳐 여러 건 있다(예: `lecture09/sections/04_rabin_karp_intro.tex:25`의 "원본의 28은 계산
오류이며 53이 정확하다"). 이는 원본 저자의 정오 기록이며 웹북 노출 방침(H3)과는 별개다.

웹북 번역은 이미 대부분 "원본" 프레이밍을 벗겨내고 알고리즘 근거만 남기고 있어 노출 문제는
없다(2026-08-07 확인, H3 참고). `07-hash-tables.qmd:102`의 "표현 교정" callout도 판단
애매로 보고됐으나 무수정으로 결론 났다 — `02_hashing_model.tex:43`의 번역이고, "원본은
틀렸다"가 아니라 흔한 오해와 정확한 모델을 대비하는 교육 장치이므로 웹북 독립성을 해치지
않는다.

다만 2026-08 세션에서 웹북이 교정하던 그림 14건을 원본에 역이식했으므로, 원본이 "원본"을
지칭하는 서술 일부는 이제 대상이 모호해졌다(예: 어떤 값을 "원본"이라 부를 때, 그 "원본"이
...
qmd 이미지 경로에 `?v=`로 시작하는 캐시 버스터를 붙이면 Quarto가 이를 정적 리소스로 인식하지
못해 `_book/`에 복사하지 않는다. `quarto render`는 정상 종료하고 HTML에도 `?v=` 문자열이 그대로
남으므로, 렌더 성공은 아무것도 보증하지 않는다. 로컬은 이전 렌더가 남긴 `_book/` 잔재 때문에
정상으로 보이고, CI의 새 빌드에서만 깨진 이미지로 드러난다. 2026-08-07 실측, 2026-08-08 재발.
SVG 재생성 후에도 경로에 아무것도 덧붙이지 마라.
